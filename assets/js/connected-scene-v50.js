(function(){
  ...
  function getProfile(){
    ...
    const fov=compact?(portrait?31.0:(landscape?32.8:32.0)):(tablet?33.0:32.4);
  ...
  function stablePixelRatio(p){
    ...
    let budget,cap,min;
    if(p.compact){
      budget=p.high?2450000:(p.low?1050000:1750000);
      cap=p.high?2.55:(p.low?1.55:2.15);
      min=p.low?1.15:1.55;
    }else if(p.tablet){
      budget=p.high?3600000:(p.low?1800000:2750000);
      cap=p.high?2.25:(p.low?1.55:1.95);
      min=p.low?1.20:1.45;
    }else{
      budget=p.high?5200000:(p.low?2600000:3900000);
      cap=p.high?2.15:(p.low?1.50:1.88);
      min=p.low?1.15:1.40;
    }
  ...
  function render(now){
    ...
    if(state.type==='service'&&performance.now()-lastScrollAt>120&&factory.animate)factory.animate(models[state.stage],now*.001+state.stage*.23);
  ...
  window.ByMeliConnectedV50={forceInit,renderAt,auditAt,getState:()=>({initialized,progress,target:targetProgress,profile:profile?{w:profile.w,h:profile.h,pixelRatio:stablePixelRatio(profile),tier:profile.tier}:null})};
