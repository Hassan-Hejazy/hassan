(function(){
  ...
  function pixelRatio(p){
    ...
    const cap=p.mobile?(p.low?1.28:(p.strong?1.82:1.58)):(p.tablet?(p.strong?1.95:1.72):(p.strong?2.15:1.9));
    const maxPixels=p.mobile?(p.low?1200000:(p.strong?2350000:1850000)):(p.tablet?(p.strong?3600000:2900000):(p.strong?5200000:4200000));
  ...
  function fitStageDistance(index,yaw,elev,margin=1){
    ...
    const usableV=profile.mobile?(profile.portrait?.57:.68):(profile.tablet?.76:.84);
    const usableH=profile.mobile?.88:(profile.tablet?.84:.72);
    ...
    return Math.max(profile.mobile?8.6:7.4,Math.max(byHeight,byWidth)*margin+halfD*.62+1.15);
  }
  ...
  function buildMaterials(){
    M.gold=new THREE.MeshStandardMaterial({color:0xd9b460,metalness:.72,roughness:.22,emissive:0x2e1a05,emissiveIntensity:.2});
    ...
    M.screen=new THREE.MeshStandardMaterial({color:0x081513,emissive:0x78d2c1,emissiveIntensity:1.35,roughness:.26});
  ...
  function shadow(mesh,cast=true){mesh.castShadow=cast&&!profile.mobile&&!profile.low;mesh.receiveShadow=!profile.mobile;return mesh;}
  ...
  function cyl(g,r1,r2,h,x,y,z,m=M.gold,seg){const n=seg||(profile.low?12:(profile.mobile?18:24));const o=shadow(new THREE.Mesh(new THREE.CylinderGeometry(r1,r2,h,n),m));o.position.set(x,y,z);g.add(o);return o;}
  function sphere(g,r,x,y,z,m=M.cream,seg){const n=seg||(profile.low?12:(profile.mobile?16:20));const o=shadow(new THREE.Mesh(new THREE.SphereGeometry(r,n,Math.max(9,n-4)),m));o.position.set(x,y,z);g.add(o);return o;}
  ...
  function createRoute(){
    ...
    for(let i=0;i<normalPositions.length-1;i++){
      ...
      const tube=new THREE.Mesh(new THREE.TubeGeometry(curve,profile.low?20:(profile.mobile?30:44),.5,profile.low?5:(profile.mobile?7:9),false),M.route.clone());
      tube.scale.y=.07;routeGroup.add(tube);
      const line=new THREE.Mesh(new THREE.TubeGeometry(curve,profile.low?20:(profile.mobile?30:44),.024,profile.low?5:7,false),M.line.clone());
      routeGroup.add(line);
  ...
  function init(){
    ...
    if(Q)Q.configureRenderer(renderer,{exposure:1.19,pixelCap:renderRatio});
    else{
    ...
    renderer.shadowMap.enabled=!profile.mobile&&!profile.low;
    ...
    key.castShadow=!profile.mobile&&!profile.low;
    const shadowSize=profile.low?768:(profile.mobile?1280:2048);
  ...
  function stageFromProgress(p){
    const mainEnd=.885;
    if(p>=mainEnd)return {overview:true,q:smoother(clamp((p-mainEnd)/(1-mainEnd))),stage:5,next:5,travel:0,hold:1};
    const scaled=clamp(p/mainEnd)*6;
    const stage=Math.min(5,Math.floor(scaled));
    const phase=scaled-stage;
    if(stage===5)return {overview:false,stage:5,next:5,travel:0,phase,hold:1};
    const travel=smoother(clamp((phase-.70)/.30));
    return {overview:false,stage,next:stage+1,travel,phase,hold:1-travel};
  }
  ...
  function applyNormalLayout(base,next,travel){
    stations.forEach((g,i)=>{
      ...
      if(base===next){
        ...
        g.visible=travel<.82;
        g.scale.setScalar(profile.displayScale*lerp(1,.94,travel));
      }else if(i===next){
        g.visible=travel>.18;
        g.scale.setScalar(profile.displayScale*lerp(.94,1,travel));
      }else{
        g.visible=false;
        g.scale.setScalar(profile.displayScale);
      }
  ...
  function stagePose(index){
    const yawMobile=[.18,-.17,.20,-.16,.16,-.20];
    const yawTablet=[.24,-.22,.25,-.20,.21,-.24];
    const yawDesktop=[.31,-.27,.32,-.23,.25,-.29];
    const elevMobile=[.16,.17,.18,.16,.15,.18];
    const elevDesktop=[.20,.21,.22,.19,.19,.21];
    ...
    if(profile.mobile||profile.portrait){
      target.y-=size.y*(profile.portrait?.28:.22);
    }else{
      target.x-=size.x*.21;
      target.y-=size.y*.035;
    }
    const distance=fitStageDistance(index,yaw,elev,profile.mobile?1.06:1.02);
  ...
  function calculateCamera(){
    ...
    if(!state.overview){
      ...
      if(state.stage===state.next){
        ...
        const inv=1-t;
        const side=state.stage%2?-.55:.55;
        tempA.copy(a.pos).lerp(b.pos,.5);
        tempA.y+=profile.mobile?1.05:1.35;
        tempA.x+=side*(profile.mobile?.58:1);
        desiredPos.set(
          inv*inv*a.pos.x+2*inv*t*tempA.x+t*t*b.pos.x,
          inv*inv*a.pos.y+2*inv*t*tempA.y+t*t*b.pos.y,
          inv*inv*a.pos.z+2*inv*t*tempA.z+t*t*b.pos.z
        );
        desiredTarget.copy(a.target).lerp(b.target,smoother(t));
        desiredTarget.y+=Math.sin(Math.PI*t)*(profile.mobile?.12:.18);
      }
    }else{
  ...
  function render(now){
    ...
    const progressDamping=reduced?1:(1-Math.exp(-(profile.mobile?13.5:15.5)*dt));
    ...
    const cameraDamping=reduced?1:(1-Math.exp(-(profile.mobile?10.5:12.5)*dt));
