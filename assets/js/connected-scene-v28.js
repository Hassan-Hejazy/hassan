(function(){
  ...
  function qualityRatio(p){
    ...
    // Ultra profile targets native-retina output on phones and a true 4K-class
    // drawing buffer on capable desktop screens, while keeping a strict pixel
    // budget on lower-tier devices so scroll performance remains stable.
    let cap,maxPixels;
    if(p.compact){
      cap=p.high?3.25:(p.low?1.55:2.35);
      maxPixels=p.high?4300000:(p.low?1450000:2950000);
    }else if(p.tablet){
      cap=p.high?2.85:(p.low?1.60:2.25);
      maxPixels=p.high?6500000:(p.low?2350000:4300000);
    }else{
      cap=p.high?2.75:(p.low?1.60:2.20);
      maxPixels=p.high?8294400:(p.low?3150000:5450000);
    }
    const pixelLimited=Math.sqrt(maxPixels/Math.max(1,p.w*p.h));
    if(p.high&&!p.compact&&!p.tablet&&p.w>=1280){
      // Supersample capable desktop displays up to an 8.29 MP (3840 x 2160)
      // drawing buffer, even when the panel DPR itself is lower.
      const fourKClass=Math.min(2.55,pixelLimited,cap);
      return Math.max(1,Math.min(cap,pixelLimited,Math.max(dpr,fourKClass)));
    }
    return Math.max(1,Math.min(dpr,cap,pixelLimited));
  }
  ...
  function makeSurfaceTexture(){
    const size=profile.high?512:256;
    const c=document.createElement('canvas');c.width=c.height=size;
    const x=c.getContext('2d');const image=x.createImageData(size,size);let seed=1729;
    const rand=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296;};
    for(let y=0;y<size;y++)for(let xx=0;xx<size;xx++){
      const i=(y*size+xx)*4;
      const broad=Math.sin(xx*.082)*4+Math.cos(y*.067)*3;
      const brushed=Math.sin((xx+y*.18)*.42)*2.2;
      const grain=(rand()-.5)*7;
      const v=Math.max(205,Math.min(252,232+broad+brushed+grain));
      image.data[i]=image.data[i+1]=image.data[i+2]=v;image.data[i+3]=255;
    }
    x.putImageData(image,0,0);
    const tex=new THREE.CanvasTexture(c);tex.wrapS=tex.wrapT=THREE.RepeatWrapping;tex.repeat.set(4.5,4.5);
  ...
  function tuneMaterials(materials){
    if(!materials)return;
    const originalBump=materials.gold&&materials.gold.bumpMap;
    const detail=surfaceTexture;
    Object.keys(materials).forEach(key=>{
      const m=materials[key];if(!m)return;
      if('envMapIntensity' in m)m.envMapIntensity=Math.min(profile.compact?1.48:1.72,Math.max(.78,m.envMapIntensity||1));
      if(detail&&'roughnessMap' in m&&key!=='glass'&&key!=='screen')m.roughnessMap=detail;
      if(detail&&'bumpMap' in m&&key!=='glass'&&key!=='screen'){
        m.bumpMap=detail;
        m.bumpScale=key==='gold'||key==='goldDark'?.0024:(key==='dark'?.0032:.0020);
      }
      m.needsUpdate=true;
    });
    if(materials.white){materials.white.color.setHex(0xf8f4ec);materials.white.roughness=.48;materials.white.metalness=.012;materials.white.envMapIntensity=.92;}
    if(materials.cream){materials.cream.color.setHex(0xeadcc3);materials.cream.roughness=.46;materials.cream.envMapIntensity=1.02;}
    if(materials.dark){materials.dark.color.setHex(0x15120f);materials.dark.roughness=.36;materials.dark.metalness=.38;materials.dark.envMapIntensity=1.22;}
    if(materials.teal){materials.teal.color.setHex(0x619d91);materials.teal.roughness=.34;materials.teal.emissiveIntensity=.060;materials.teal.envMapIntensity=1.28;materials.teal.clearcoat=.28;materials.teal.clearcoatRoughness=.16;}
    if(materials.red){materials.red.roughness=.42;materials.red.envMapIntensity=.92;}
    if(materials.glass){
      materials.glass.transmission=0;materials.glass.opacity=profile.compact?.34:.40;materials.glass.roughness=profile.compact?.085:.060;
      materials.glass.metalness=.02;materials.glass.depthWrite=false;materials.glass.envMapIntensity=1.62;materials.glass.clearcoat=.62;materials.glass.clearcoatRoughness=.035;
    }
    if(materials.gold){materials.gold.color.setHex(0xd9ad58);materials.gold.roughness=.24;materials.gold.metalness=.80;materials.gold.clearcoat=profile.compact?.56:.68;materials.gold.clearcoatRoughness=.085;materials.gold.envMapIntensity=profile.compact?1.58:1.82;}
    if(materials.goldDark){materials.goldDark.color.setHex(0x83602e);materials.goldDark.roughness=.34;materials.goldDark.metalness=.64;materials.goldDark.envMapIntensity=1.42;}
    if(materials.screen){materials.screen.emissiveIntensity=1.34;materials.screen.roughness=.13;materials.screen.metalness=.035;materials.screen.envMapIntensity=.84;}
    Object.keys(materials).forEach(key=>materials[key]&&(materials[key].needsUpdate=true));
    if(originalBump&&originalBump!==detail&&originalBump.dispose)originalBump.dispose();
  }
  ...
  function init(){
    ...
    if(Q)Q.configureRenderer(renderer,{exposure:1.27,pixelCap:renderRatio});
    ...
    const connectedShadows=profile.high||(!profile.compact&&!profile.low);
    ...
    if(keyLight.castShadow){const size=profile.compact?(profile.high?1024:768):(profile.high?2048:1280);keyLight.shadow.mapSize.set(size,size);keyLight.shadow.camera.left=-10;keyLight.shadow.camera.right=10;keyLight.shadow.camera.top=10;keyLight.shadow.camera.bottom=-10;keyLight.shadow.bias=-.00024;keyLight.shadow.normalBias=.024;keyLight.shadow.radius=2.2;}
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V28 failed:',error);
  ...
  window.ByMeliConnectedV28={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio}:null})};
  window.ByMeliConnectedV27=window.ByMeliConnectedV28;
  window.ByMeliConnectedV26=window.ByMeliConnectedV28;
  window.ByMeliConnectedV25=window.ByMeliConnectedV28;
  window.ByMeliConnectedV24=window.ByMeliConnectedV28;
  window.ByMeliConnectedV20=window.ByMeliConnectedV28;
