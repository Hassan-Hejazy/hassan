(function(){
  ...
  function getProfile(){
    ...
    let fov=31.8;
    if(compact)fov=landscape?31.6:(portrait?30.6:31.2);
    else if(tablet)fov=32.6;
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      ...
      motionPixels=p.high?1900000:(p.low?920000:1450000);
      ultraPixels=p.high?7000000:(p.low?2450000:4800000);
      motionCap=p.high?2.02:(p.low?1.46:1.82);
      ultraCap=p.high?3.18:(p.low?2.02:2.84);
      minRatio=p.low?1.10:1.34;
    }else if(p.tablet){
  ...
  function configureOverview(){
    if(profile.compact&&profile.portrait){
      overviewPositions=[
        new THREE.Vector3(-4.95,0,7.40),new THREE.Vector3(4.95,0,7.40),
        new THREE.Vector3(-4.95,0,0),new THREE.Vector3(4.95,0,0),
        new THREE.Vector3(-4.95,0,-7.40),new THREE.Vector3(4.95,0,-7.40)
      ];
    }else if(profile.tablet||profile.w<1250){
      overviewPositions=[
        new THREE.Vector3(-5.80,0,6.70),new THREE.Vector3(5.80,0,6.70),
        new THREE.Vector3(-5.80,0,0),new THREE.Vector3(5.80,0,0),
        new THREE.Vector3(-5.80,0,-6.70),new THREE.Vector3(5.80,0,-6.70)
      ];
    }else{
      overviewPositions=[
        new THREE.Vector3(-8.20,0,5.90),new THREE.Vector3(0,0,6.15),new THREE.Vector3(8.20,0,5.90),
        new THREE.Vector3(-8.20,0,-5.90),new THREE.Vector3(0,0,-6.15),new THREE.Vector3(8.20,0,-5.90)
      ];
    }
    updateOverviewLinks();
  ...
  function buildLuxuryStudio(){
    ...
    if(Q)Q.addContactShadow(studio,renderer,profile.compact?10.0:12.4,profile.compact?.42:.44,.014);
  }
  ...
  function stageYaw(i){const base=stagePreset(i).yaw;const lead=profile.compact?(profile.portrait?.012:.018):(profile.tablet?.025:.030);return base-lead;}

  function stageElevation(i){ return stagePreset(i).elev-(profile.compact?(profile.portrait?.052:.036):(profile.tablet?.018:.012)); }

  function stageComposition(i){ const p=stagePreset(i);return {x:0,y:p.compY+(profile.compact?(profile.portrait?-.155:-.092):.010)}; }
  ...
  function holdPose(i,u){
    ...
    const orbit=profile.compact?.002:.007;
    ...
    const pose=framedPose(box,yaw,elev,profile.fov,profile.compact?.0008:.006,stageComposition(i),focus);
    const dolly=1-Math.sin(Math.PI*u)*(profile.compact?.002:.005);
  ...
  function transitionLayout(i,q){
    ...
    const shift=i===3?(profile.compact?.05:.12):(profile.compact?.10:.20);
    const depth=profile.compact?.03:.07;
    return {
  ...
  function bridgePose(i,q){
    ...
    const side=i%2?-1:1;
    pos.y+=Math.sin(Math.PI*t)*(profile.compact?.05:.10);
    pos.x+=Math.sin(Math.PI*t)*side*(profile.compact?.025:.05);
    pos.z+=Math.sin(Math.PI*t)*(profile.compact?.035:.07);
  ...
  function finalOverviewScale(){
    if(profile.compact&&profile.portrait)return .30;
    if(profile.compact)return .34;
    if(profile.tablet||profile.w<1250)return .36;
    return .39;
  }
  ...
  function finalOverviewPose(){
    ...
    stations.forEach((_,i)=>union.union(transformedBox(i,overviewCenter.clone().add(overviewPositions[i]),scale,false,SERVICE_RELATIVE_YAW[i]*.75)));
    const yaw=portrait?0.0:(twoColumn?.018:.095);
    const elev=portrait?.86:(twoColumn?.72:(profile.compact?.60:.54));
    const fov=portrait?(profile.fov+1.8):(twoColumn?(profile.fov+1.1):(profile.fov+.4));
    return framedPose(union,yaw,elev,fov,portrait?.014:(twoColumn?.018:.016),{x:0,y:portrait?.010:.006});
  }
    ...
    if(state.mode==='hold'){
      ...
      if(state.q>.20){
        stations.forEach((s,i)=>{
          ...
          const overviewYaw=SERVICE_RELATIVE_YAW[i]*.75;
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V47 failed:',error);
  ...
  window.ByMeliConnectedV47={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),startupMs:initReadyAt&&initStartedAt?initReadyAt-initStartedAt:null,visible,rafActive:Boolean(rafId),pageVisible,profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV46=window.ByMeliConnectedV47;
  window.ByMeliConnectedV45=window.ByMeliConnectedV47;
  window.ByMeliConnectedV44=window.ByMeliConnectedV47;
  window.ByMeliConnectedV43=window.ByMeliConnectedV47;
  window.ByMeliConnectedV42=window.ByMeliConnectedV47;
  window.ByMeliConnectedV41=window.ByMeliConnectedV47;
  window.ByMeliConnectedV40=window.ByMeliConnectedV47;
  window.ByMeliConnectedV39=window.ByMeliConnectedV47;
  window.ByMeliConnectedV38=window.ByMeliConnectedV47;
  window.ByMeliConnectedV37=window.ByMeliConnectedV47;
  window.ByMeliConnectedV36=window.ByMeliConnectedV47;
  window.ByMeliConnectedV35=window.ByMeliConnectedV47;
  window.ByMeliConnectedV34=window.ByMeliConnectedV47;
  window.ByMeliConnectedV33=window.ByMeliConnectedV47;
  window.ByMeliConnectedV32=window.ByMeliConnectedV47;
  window.ByMeliConnectedV31=window.ByMeliConnectedV47;
  window.ByMeliConnectedV30=window.ByMeliConnectedV47;
  window.ByMeliConnectedV29=window.ByMeliConnectedV47;
  window.ByMeliConnectedV28=window.ByMeliConnectedV47;
  window.ByMeliConnectedV27=window.ByMeliConnectedV47;
  window.ByMeliConnectedV26=window.ByMeliConnectedV47;
  window.ByMeliConnectedV25=window.ByMeliConnectedV47;
  window.ByMeliConnectedV24=window.ByMeliConnectedV47;
  window.ByMeliConnectedV20=window.ByMeliConnectedV47;
