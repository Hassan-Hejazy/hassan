(function(){
  ...
  function transitionLayout(i,q){
    ...
    const gather=(profile.compact?.86:.82)*blend;
    const aPos=routePositions[i].clone().lerp(mid,gather);
    const bPos=routePositions[i+1].clone().lerp(mid,gather);
    const minimumScale=profile.compact?.82:.85;
    const scale=lerp(1,minimumScale,blend);
    return {t,blend,aPos,bPos,aScale:scale,bScale:scale};
  }

  function bridgePose(i,q){
    const layout=transitionLayout(i,q);
    const union=new THREE.Box3();
    if(q<.78)union.union(transformedBox(i,layout.aPos,layout.aScale,false,0));
    if(q>.22)union.union(transformedBox(i+1,layout.bPos,layout.bScale,false,0));
    if(union.isEmpty()){
      const fallbackIndex=q<.5?i:i+1;
      union.copy(stageWorldBox(fallbackIndex));
    }
    const yaw=lerp(stageYaw(i),stageYaw(i+1),layout.t);
    const elev=lerp(stageElevation(i),stageElevation(i+1),layout.t)+layout.blend*(profile.compact?.006:.009);
    const focus=union.getCenter(new THREE.Vector3());
    const size=union.getSize(new THREE.Vector3());
    focus.y+=size.y*(profile.compact?.012:.009);
    const pose=framedPose(
      union,
      yaw,
      elev,
      profile.fov+layout.blend*(profile.compact?.10:.14),
      profile.compact?.014:.020,
      {x:0,y:lerp(stageComposition(i).y,stageComposition(i+1).y,layout.t)},
      focus
    );
    pose.pos.y+=layout.blend*(profile.compact?.035:.055);
    return pose;
  }
  ...
  function applyState(state){
    ...
    if(state.mode==='hold'){
      ...
      a.wrapper.visible=state.q<.78;b.wrapper.visible=state.q>.22;
      ...
      a.wrapper.rotation.y=0;b.wrapper.rotation.y=0;
      a.wrapper.scale.setScalar(layout.aScale);b.wrapper.scale.setScalar(layout.bScale);
      a.accent.intensity=lerp(profile.compact?.86:1.02,.28,t);b.accent.intensity=lerp(.28,profile.compact?.86:1.02,t);
      a.scanner.group.visible=state.q<.48;b.scanner.group.visible=state.q>.52;
    }else{
      if(state.q<.56){
      ...
      if(state.q>.20){
        stations.forEach((s,i)=>{
          ...
          setStationLOD(s,'overview');s.wrapper.visible=true;s.detail.visible=true;s.preview.visible=false;s.wrapper.position.copy(p.pos);s.wrapper.scale.setScalar(p.scale);s.wrapper.rotation.y=0;
        });
      }
    }
    if(hub){
  ...
  function animateScene(now,state,dt){
    ...
    dataPackets.forEach((p,i)=>{const u=(t*p.userData.speed+p.userData.offset)%1;p.position.copy(p.userData.curve.getPointAt(u));p.position.y+=Math.sin(t*3+i)*.018;p.material.opacity=.58+.24*Math.sin(Math.PI*u);});
    journeyPackets.forEach((p,i)=>{const u=(t*p.userData.speed+p.userData.offset)%1;p.position.copy(p.userData.curve.getPointAt(u));p.position.y+=Math.sin(t*2.6+i)*.014;p.material.opacity=.54+.26*Math.sin(Math.PI*u);});
    ...
    stations.forEach((s,i)=>{
      ...
      if(isActive&&now-s.screen.lastDrawAt>420){s.screen.lastDrawAt=now;drawScreen(s.screen,now,true,false);}
    });
    if(now-screenUpdateAt>1100){screenUpdateAt=now;stations.forEach((s,i)=>{if(s.wrapper.visible&&i!==active)drawScreen(s.screen,now,false,overview);});}
  ...
  function render(now){
    ...
    const lambda=(profile.compact?24.0:22.0)+Math.min(6.0,gap*16);
  ...
  function disposeConnected(replace=true){
    ...
    initialized=false;initQueued=false;stableFrames=0;lastStateKey='';
  ...
  function unlockServiceContexts(phase='ready'){
    contextLock=false;
  ...
  function init(){
    ...
    if(pose){camera.position.copy(desiredPos);camera.up.set(0,1,0);camera.lookAt(desiredTarget);camera.fov=pose.fov;camera.updateProjectionMatrix();}
    bindCanvasLoss();bindGlobal();readProgress();progress=targetProgress;initAttempts=0;unlockServiceContexts('ready');if(visible)startLoop();
  }
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      if(initialized){unlockServiceContexts('already-ready');return;}
      try{init();}
      catch(error){
        console.error('[By Meli] Connected V25 failed:',error);
  ...
  window.ByMeliConnectedV25={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio}:null})};
  window.ByMeliConnectedV24=window.ByMeliConnectedV25;
  window.ByMeliConnectedV20=window.ByMeliConnectedV25;
