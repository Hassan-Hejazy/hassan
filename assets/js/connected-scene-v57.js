(function(){
  ...
  function getProfile(){
    ...
    const fov=compact?(portrait?29.8:(landscape?31.2:30.8)):(tablet?32.2:31.7);
  ...
  function stablePixelRatio(p){
    ...
    let budget,cap,min;
    if(p.compact){
      budget=p.high?2550000:(p.low?1200000:1900000);
      cap=p.high?2.35:(p.low?1.58:2.05);
      min=p.low?1.14:1.42;
    }else if(p.tablet){
  ...
  function createRenderer(){
    try{return new THREE.WebGLRenderer({canvas,antialias:(!profile.compact)||(profile.compact&&profile.high),alpha:false,premultipliedAlpha:false,powerPreference:(profile.compact&&!profile.high)?'default':'high-performance',precision:(profile.compact&&!profile.high)?'mediump':'highp',stencil:false,preserveDrawingBuffer:false,depth:true});}
    catch(_){
  ...
  function init(){
    ...
    try{
      ...
      console.error('Connected V57 init failed:',error);
      initialized=false;
  ...
  window.ByMeliConnectedV57={forceInit,renderAt,auditAt,getState:()=>({initialized,progress,target:targetProgress,profile:profile?{w:profile.w,h:profile.h,pixelRatio:stablePixelRatio(profile),tier:profile.tier}:null})};
  for(const v of [56,55,54,53,52,51,50,49,48,47,46,45,44,43,42,41,40,39,38,37,36,35,34,33,32,31,30,29,28,27,26,25,24,20])window['ByMeliConnectedV'+v]=window.ByMeliConnectedV57;
