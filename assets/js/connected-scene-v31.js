(function(){
  ...
  let stableViewportHeight=0,lastLayoutWidth=0,lastLayoutHeight=0,lastOrientation='',resizeForce=false;
  ...
  function captureStableViewport(force=false){
    const w=Math.max(1,Math.round(window.innerWidth||document.documentElement.clientWidth||sticky.clientWidth||1));
    const h=Math.max(1,Math.round(window.innerHeight||document.documentElement.clientHeight||sticky.clientHeight||1));
    const orientation=w>h?'landscape':'portrait';
    const widthChanged=Math.abs(w-lastLayoutWidth)>2;
    const orientationChanged=orientation!==lastOrientation;
    if(force||!stableViewportHeight||widthChanged||orientationChanged||(!coarse&&Math.abs(h-lastLayoutHeight)>2)){
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      cap=p.high?2.80:(p.low?1.62:2.35);
      targetWidth=p.landscape?2560:1440;
      targetHeight=2160;
      ultraPixels=p.high?4100000:(p.low?1800000:3200000);
      motionPixels=p.high?1800000:(p.low?1150000:1500000);
      minRatio=p.low?1.18:1.45;
    }else if(p.tablet){
      cap=p.high?2.75:(p.low?1.72:2.38);
      targetWidth=3200;
      targetHeight=2160;
      ultraPixels=p.high?6900000:(p.low?3000000:5200000);
      motionPixels=p.high?5000000:(p.low?2250000:3850000);
      minRatio=p.low?1.22:1.48;
    }else{
      cap=p.high?2.75:(p.low?1.78:2.42);
      targetWidth=3840;
      targetHeight=2160;
      ultraPixels=p.high?8350000:(p.low?3500000:6300000);
      motionPixels=p.high?6400000:(p.low?2500000:4700000);
      minRatio=p.low?1.24:1.50;
    }
  ...
  function updateAdaptiveQuality(now,frameMs,scrollActive){
    if(scrollActive){
      ...
      if(average>27&&motionRenderRatio>minRenderRatio+.10&&now-lastQualityChange>1800){
    ...
    if(now-lastScrollAt>210&&qualityState!=='ultra')useUltraQuality();
  }
  ...
  function createRenderer(){
    const options={canvas,antialias:!profile.low,alpha:false,premultipliedAlpha:false,powerPreference:'high-performance',precision:profile.low?'mediump':'highp',stencil:false,preserveDrawingBuffer:false,depth:true,logarithmicDepthBuffer:false,desynchronized:true,failIfMajorPerformanceCaveat:false};
    try{return new THREE.WebGLRenderer(options);}
    catch(_){
      try{return new THREE.WebGLRenderer(Object.assign({},options,{antialias:false,powerPreference:'default',precision:'mediump',desynchronized:false}));}
    ...
    const io=new IntersectionObserver(entries=>{
      ...
      if(visible){
        ...
        disposeTimer=setTimeout(()=>{
          ...
          const rect=track.getBoundingClientRect(),vh=stableViewportHeight||window.innerHeight||1;
          if(rect.bottom < -vh*3.0 || rect.top > vh*3.5)disposeConnected(true);
        },5200);
      }
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V31 failed:',error);
  ...
  window.ByMeliConnectedV31={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV30=window.ByMeliConnectedV31;
  window.ByMeliConnectedV29=window.ByMeliConnectedV31;
  window.ByMeliConnectedV28=window.ByMeliConnectedV31;
  window.ByMeliConnectedV27=window.ByMeliConnectedV31;
  window.ByMeliConnectedV26=window.ByMeliConnectedV31;
  window.ByMeliConnectedV25=window.ByMeliConnectedV31;
  window.ByMeliConnectedV24=window.ByMeliConnectedV31;
  window.ByMeliConnectedV20=window.ByMeliConnectedV31;
  const bootstrap=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting)){bootstrap.disconnect();queueInit();}},{rootMargin:'1600px 0px',threshold:0});bootstrap.observe(track);
