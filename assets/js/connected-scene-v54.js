(function(){
  ...
  function transitionPose(fromStage,toStage,q){
    const t=clamp(q);
    const a=servicePoseAt(fromStage,1);
    const b=servicePoseAt(toStage,0);
    const pos=a.pos.clone().lerp(b.pos,t);
    const target=a.target.clone().lerp(b.target,t);
    return {pos,target,fov:lerp(a.fov,b.fov,t)};
  }
  ...
  function init(){
    ...
    try{
      ...
      console.error('Connected V54 init failed:',error);
      initialized=false;
  ...
  window.ByMeliConnectedV54={forceInit,renderAt,auditAt,getState:()=>({initialized,progress,target:targetProgress,profile:profile?{w:profile.w,h:profile.h,pixelRatio:stablePixelRatio(profile),tier:profile.tier}:null})};
  for(const v of [53,52,51,50,49,48,47,46,45,44,43,42,41,40,39,38,37,36,35,34,33,32,31,30,29,28,27,26,25,24,20])window['ByMeliConnectedV'+v]=window.ByMeliConnectedV54;
