(function(){
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      cap=p.high?3.60:(p.low?1.85:2.85);
      targetWidth=p.landscape?3840:2160;
      targetHeight=p.landscape?2160:3840;
      ultraPixels=p.high?8200000:(p.low?2600000:5200000);
      motionPixels=p.high?3600000:(p.low?1350000:2350000);
      minRatio=p.low?1.22:1.65;
    }else if(p.tablet){
      cap=p.high?3.35:(p.low?1.90:2.90);
      targetWidth=3840;targetHeight=2160;
      ultraPixels=p.high?9000000:(p.low?3400000:6500000);
      motionPixels=p.high?5200000:(p.low?2100000:3600000);
      minRatio=p.low?1.28:1.70;
    }else{
      cap=p.high?3.25:(p.low?1.95:2.95);
      targetWidth=3840;targetHeight=2160;
      ultraPixels=p.high?10000000:(p.low?4000000:7600000);
      motionPixels=p.high?6600000:(p.low?2700000:4700000);
      minRatio=p.low?1.32:1.75;
    }
  ...
  function configureRoute(){
    routeRadius=0;
    const sx=profile.compact?(profile.portrait?4.6:5.25):(profile.tablet?5.7:6.3);
    const sz=profile.compact?(profile.portrait?4.25:4.05):(profile.tablet?4.45:4.8);
    routePositions=[
  ...
  function buildIntegratedWorld(){
    ...
    const minX=routePositions[0].x-5.5,maxX=routePositions[5].x+5.5,width=maxX-minX;
    ...
    const floor=new THREE.Mesh(new THREE.BoxGeometry(width,.24,15.8),floorMat);floor.position.set((minX+maxX)/2,-.18,0);floor.receiveShadow=renderer.shadowMap.enabled;integratedWorld.add(floor);
    ...
    const aisle=new THREE.Mesh(new THREE.BoxGeometry(width-.7,.035,3.15),aisleMat);aisle.position.set((minX+maxX)/2,-.035,3.45);aisle.receiveShadow=renderer.shadowMap.enabled;integratedWorld.add(aisle);
    const aisleBack=aisle.clone();aisleBack.position.z=-3.45;integratedWorld.add(aisleBack);
    ...
    [-1,1].forEach(sign=>{const line=new THREE.Mesh(new THREE.BoxGeometry(width-.8,.018,.045),borderMat);line.position.set((minX+maxX)/2,.005,3.45+sign*1.56);integratedWorld.add(line);worldRings.push(line);});
    const pathPoints=routePositions.map((p,i)=>new THREE.Vector3(p.x,.08,p.z+2.65+(i%2?.12:-.12)));
    ...
    routePositions.forEach((p,i)=>{
      ...
      const back=new THREE.Mesh(new THREE.BoxGeometry(7.45,4.85,.18),wallMat);back.position.set(0,2.38,-4.05);back.receiveShadow=renderer.shadowMap.enabled;architecture.add(back);
      for(let k=-3;k<=3;k++){const slat=new THREE.Mesh(new THREE.BoxGeometry(.035,4.25,.035),bronze);slat.position.set(k*1.03,2.35,-3.94);architecture.add(slat);}
      const left=new THREE.Mesh(new THREE.BoxGeometry(.18,5.35,.34),bronze),right=left.clone(),top=new THREE.Mesh(new THREE.BoxGeometry(7.65,.18,.34),bronze);
      left.position.set(-3.73,2.68,-3.82);right.position.set(3.73,2.68,-3.82);top.position.set(0,5.28,-3.82);architecture.add(left,right,top);worldFrames.push(left,right,top);
      const light=new THREE.Mesh(new THREE.BoxGeometry(6.65,.035,.035),glowMat);light.position.set(0,5.13,-3.62);architecture.add(light);
      const plinth=new THREE.Mesh(new THREE.CylinderGeometry(4.35,4.55,.14,profile.low?64:128),new THREE.MeshPhysicalMaterial({color:0x12120f,roughness:.30,metalness:.38,clearcoat:.34,clearcoatRoughness:.18,envMapIntensity:1.22}));plinth.position.y=-.035;plinth.receiveShadow=renderer.shadowMap.enabled;portal.add(plinth);
      const halo=new THREE.Mesh(new THREE.TorusGeometry(4.08,.027,8,profile.low?72:144),new THREE.MeshBasicMaterial({color:i%2?0x69cbbb:0xe7c06a,transparent:true,opacity:.34,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));halo.rotation.x=Math.PI/2;halo.position.y=.055;portal.add(halo);worldRings.push(halo);
      const spotColor=i%2?0x7ddbc9:0xffd98a;
      const spot=new THREE.SpotLight(spotColor,profile.compact?2.8:3.6,17,.48,.62,1.45);spot.position.set(1.65,6.2,1.9);spot.target.position.set(0,1.25,0);spot.castShadow=renderer.shadowMap.enabled&&!profile.compact;portal.add(spot,spot.target);worldBeacons.push(spot);
      const rim=new THREE.PointLight(i%2?0x68cbbb:0xe6b95d,profile.compact?.42:.62,11,2);rim.position.set(-2.7,2.6,-.6);portal.add(rim);worldBeacons.push(rim);
    ...
    [-5.75,5.75].forEach(z=>{const beam=new THREE.Mesh(new THREE.BoxGeometry(width,.10,.16),bronze);beam.position.set((minX+maxX)/2,6.18,z);integratedWorld.add(beam);worldFrames.push(beam);});
    for(let x=minX+2;x<maxX-1;x+=3.1){const cross=new THREE.Mesh(new THREE.BoxGeometry(.10,.10,11.6),bronze);cross.position.set(x,6.18,0);integratedWorld.add(cross);worldFrames.push(cross);const lamp=new THREE.PointLight(0xffe0a8,profile.compact?.14:.24,7,2);lamp.position.set(x,5.9,3.25);integratedWorld.add(lamp);worldBeacons.push(lamp);}
  }
  ...
  window.ByMeliConnectedV35={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV34=window.ByMeliConnectedV35;
  window.ByMeliConnectedV33=window.ByMeliConnectedV35;
  window.ByMeliConnectedV32=window.ByMeliConnectedV35;
  window.ByMeliConnectedV31=window.ByMeliConnectedV35;
  window.ByMeliConnectedV30=window.ByMeliConnectedV35;
  window.ByMeliConnectedV29=window.ByMeliConnectedV35;
  window.ByMeliConnectedV28=window.ByMeliConnectedV35;
  window.ByMeliConnectedV27=window.ByMeliConnectedV35;
  window.ByMeliConnectedV26=window.ByMeliConnectedV35;
  window.ByMeliConnectedV25=window.ByMeliConnectedV35;
  window.ByMeliConnectedV24=window.ByMeliConnectedV35;
  window.ByMeliConnectedV20=window.ByMeliConnectedV35;
