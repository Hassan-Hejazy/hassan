(function(){
  ...
  function buildIntegratedWorld(){
    ...
    const routeGlow=new THREE.Mesh(new THREE.TubeGeometry(journeyRoute,profile.low?100:220,.085,8,false),new THREE.MeshBasicMaterial({color:0x69cdbb,transparent:true,opacity:.13,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));integratedWorld.add(routeGlow);worldRings.push(routeGlow);
    const routeCore=new THREE.Mesh(new THREE.TubeGeometry(journeyRoute,profile.low?100:220,.018,6,false),new THREE.MeshBasicMaterial({color:0xe7c36e,transparent:true,opacity:.72,depthWrite:false,toneMapped:false}));integratedWorld.add(routeCore);worldRings.push(routeCore);
  ...
  function finalOverviewScale(){return 1;}

  function finalOverviewPose(){
    const union=new THREE.Box3();stations.forEach((_,i)=>union.union(transformedBox(i,routePositions[i],1,false,stationRotations[i]||0)));
    // The finale is a clean architectural hero shot, not a technical top view.
    // Include the plinth footprint but exclude overhead truss and portal walls.
    union.expandByPoint(new THREE.Vector3(routePositions[0].x-4.8,-.20,-5.25));
    union.expandByPoint(new THREE.Vector3(routePositions[2].x+4.8,4.75,5.25));
    const focus=union.getCenter(new THREE.Vector3());focus.y=.98;
    const portrait=profile.compact&&profile.portrait;
    const yaw=portrait?.065:(profile.compact?.10:.16);
    const elev=portrait?.66:(profile.compact?.57:(profile.tablet?.54:.50));
    const fov=portrait?34.5:(profile.compact?33.5:(profile.tablet?31.5:29.5));
    return framedPose(union,yaw,elev,fov,profile.compact?.030:.025,{x:0,y:portrait?-.010:-.018},focus);
  }
  ...
  function finalPlacement(i,q){
    ...
    return {pos:routePositions[i].clone(),scale:1,reveal};
  }


  function transitionVeil(state){
    if(state.mode==='transition')return Math.pow(Math.sin(Math.PI*clamp(state.q)),1.45);
    if(state.mode==='overview'){const q=clamp(state.q);if(q<.18)return q/.18*.18;if(q<.76)return .18+Math.sin(Math.PI*(q-.18)/.58)*.46;return (1-q)/.24*.18;}
    return 0;
  }
  ...
  function applyState(state){
    ...
    if(integratedWorld){
      ...
      worldPortals.forEach((portal,i)=>{
        ...
        else if(state.mode==='transition'){const cut=state.q>.44&&state.q<.56;portal.visible=!cut&&(i===state.stage||i===state.next);}
    ...
    if(state.mode==='hold'){
      ...
      if(state.q<.10){
        ...
        stations.forEach((s,i)=>{const p=finalPlacement(i,state.q);if(p.reveal<=.004)return;setStationLOD(s,'overview');s.wrapper.visible=true;s.detail.visible=true;s.wrapper.position.copy(routePositions[i]);s.wrapper.scale.setScalar(1);s.wrapper.rotation.y=stationRotations[i]||0;s.accent.intensity=.24*p.reveal;});
      }
    }
  ...
  window.ByMeliConnectedV36={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV35=window.ByMeliConnectedV36;
  window.ByMeliConnectedV34=window.ByMeliConnectedV36;
  window.ByMeliConnectedV33=window.ByMeliConnectedV36;
  window.ByMeliConnectedV32=window.ByMeliConnectedV36;
  window.ByMeliConnectedV31=window.ByMeliConnectedV36;
  window.ByMeliConnectedV30=window.ByMeliConnectedV36;
  window.ByMeliConnectedV29=window.ByMeliConnectedV36;
  window.ByMeliConnectedV28=window.ByMeliConnectedV36;
  window.ByMeliConnectedV27=window.ByMeliConnectedV36;
  window.ByMeliConnectedV26=window.ByMeliConnectedV36;
  window.ByMeliConnectedV25=window.ByMeliConnectedV36;
  window.ByMeliConnectedV24=window.ByMeliConnectedV36;
  window.ByMeliConnectedV20=window.ByMeliConnectedV36;
