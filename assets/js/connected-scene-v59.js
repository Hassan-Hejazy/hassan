(function(){
  ...
  function getProfile(){
    ...
    const low=memory<=2||(memory<=3&&cores<=4);
    const high=(memory>=6&&cores>=6)||(memory>=4&&cores>=6&&(window.devicePixelRatio||1)>=3);
    const tier=low?'low':(high?'high':'balanced');
    const fov=compact?(portrait?29.2:(landscape?30.6:30.2)):(tablet?32.2:31.7);
  ...
  function stablePixelRatio(p){
    ...
    let budget,cap,min;
    if(p.compact){
      budget=p.high?3100000:(p.low?1700000:2300000);
      cap=p.high?2.55:(p.low?1.82:2.20);
      min=p.low?1.34:1.58;
    }else if(p.tablet){
      budget=p.high?3300000:(p.low?1800000:2550000);
      cap=p.high?2.08:(p.low?1.52:1.84);
      min=p.low?1.10:1.32;
    }else{
      budget=p.high?4400000:(p.low?2400000:3450000);
  ...
  function createRenderer(){
    try{return new THREE.WebGLRenderer({canvas,antialias:(!profile.low),alpha:false,premultipliedAlpha:false,powerPreference:profile.low?'default':'high-performance',precision:profile.low?'mediump':'highp',stencil:false,preserveDrawingBuffer:false,depth:true});}
    catch(_){
  ...
  function safeFrame(){
    ...
    if(profile.compact&&profile.portrait)return {left:-.96,right:.96,bottom:-.10,top:.76};
    if(profile.compact&&profile.landscape)return rtl?{left:-.95,right:-.02,bottom:-.72,top:.82}:{left:.02,right:.95,bottom:-.72,top:.82};
    if(profile.compact)return {left:-.96,right:.96,bottom:-.16,top:.78};
  ...
  function transitionPose(fromStage,toStage,q){
    ...
    const distance=retreat*(profile.compact?.20:.42);
    const pos=base.pos.clone().addScaledVector(direction,distance);
    pos.y+=retreat*(profile.compact?.026:.058);
    return {pos,target:base.target.clone(),fov:base.fov+retreat*(profile.compact?.10:.18)};
  }
  ...
  function render(now){
    ...
    if(renderDirty||performance.now()-lastScrollAt<110)raf=requestAnimationFrame(render);
  }
  ...
  function init(){
    ...
    try{
      ...
      initialized=true;canvas.style.display='block';renderDirty=true;
      bind();visible=true;startLoop();
    }catch(error){
      console.error('Connected V59 init failed:',error);
      initialized=false;
  ...
  function bind(){
    if(observer)return;
    const onScroll=()=>{readProgress();lastScrollAt=performance.now();renderDirty=true;startLoop();};
    window.addEventListener('scroll',onScroll,{passive:true});
    window.addEventListener('resize',scheduleResize,{passive:true});
    window.addEventListener('orientationchange',()=>setTimeout(scheduleResize,180),{passive:true});
    window.addEventListener('pageshow',()=>{pageVisible=true;visible=true;renderDirty=true;scheduleResize();startLoop();},{passive:true});
    ...
    document.addEventListener('visibilitychange',()=>{pageVisible=!document.hidden;if(pageVisible&&visible){renderDirty=true;startLoop();}else stopLoop();});
    ...
    if('IntersectionObserver'in window){
      observer=new IntersectionObserver(entries=>{
        ...
        if(visible){renderDirty=true;startLoop();}else stopLoop();
    ...
    document.addEventListener('languagechange',()=>{copyStage=-1;renderDirty=true;startLoop();});
  ...
  window.ByMeliConnectedV59={forceInit,renderAt,auditAt,getState:()=>({initialized,progress,target:targetProgress,profile:profile?{w:profile.w,h:profile.h,pixelRatio:stablePixelRatio(profile),tier:profile.tier}:null})};
  for(const v of [58,57,56,55,54,53,52,51,50,49,48,47,46,45,44,43,42,41,40,39,38,37,36,35,34,33,32,31,30,29,28,27,26,25,24,20])window['ByMeliConnectedV'+v]=window.ByMeliConnectedV59;

  if('IntersectionObserver'in window){
    const io=new IntersectionObserver(entries=>{
      if(entries.some(e=>e.isIntersecting)){io.disconnect();forceInit();}
    },{rootMargin:'2400px 0px',threshold:0});io.observe(track);
  }else{
    setTimeout(forceInit,80);
  }
  if(document.readyState==='complete')setTimeout(forceInit,120);
  window.addEventListener('load',()=>setTimeout(forceInit,60),{once:true});
  if('requestIdleCallback'in window)requestIdleCallback(forceInit,{timeout:700});else setTimeout(forceInit,350);
})();
