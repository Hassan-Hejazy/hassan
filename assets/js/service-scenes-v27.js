(function(){
  ...
  function makeDetailTexture(renderer){
    const size=128,c=document.createElement('canvas');c.width=c.height=size;
    const x=c.getContext('2d'),img=x.createImageData(size,size);let seed=918273;
    const rand=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296;};
    for(let y=0;y<size;y++)for(let xx=0;xx<size;xx++){
      const i=(y*size+xx)*4;const grain=(rand()-.5)*24;const weave=Math.sin(xx*.48)*3+Math.cos(y*.41)*3;
      const v=Math.max(92,Math.min(166,128+grain+weave));img.data[i]=img.data[i+1]=img.data[i+2]=v;img.data[i+3]=255;
    }
    x.putImageData(img,0,0);
    const tex=new THREE.CanvasTexture(c);tex.wrapS=tex.wrapT=THREE.RepeatWrapping;tex.repeat.set(4,4);tex.generateMipmaps=true;tex.minFilter=THREE.LinearMipmapLinearFilter;tex.magFilter=THREE.LinearFilter;
    if(renderer?.capabilities?.getMaxAnisotropy)tex.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());
  ...
  function createMaterials(renderer,options={}){
    ...
    const detailTexture=makeDetailTexture(renderer);
    return {
      gold:new THREE.MeshPhysicalMaterial({color:0xd4ad5d,metalness:.74,roughness:.22,clearcoat:.42,clearcoatRoughness:.16,emissive:0x352006,emissiveIntensity:.11,envMapIntensity:1.42,bumpMap:detailTexture,bumpScale:.008}),
      goldDark:new THREE.MeshPhysicalMaterial({color:0x765527,metalness:.56,roughness:.34,clearcoat:.24,clearcoatRoughness:.24,envMapIntensity:1.08,bumpMap:detailTexture,bumpScale:.010}),
      dark:new THREE.MeshStandardMaterial({color:0x1c1814,metalness:.30,roughness:.44,envMapIntensity:.92,bumpMap:detailTexture,bumpScale:.014}),
      cream:new THREE.MeshStandardMaterial({color:0xeee2ca,metalness:.03,roughness:.54,envMapIntensity:.64,bumpMap:detailTexture,bumpScale:.012}),
      white:new THREE.MeshStandardMaterial({color:0xf8f4eb,metalness:.02,roughness:.58,envMapIntensity:.58,bumpMap:detailTexture,bumpScale:.010}),
      teal:new THREE.MeshPhysicalMaterial({color:0x609f92,metalness:.22,roughness:.31,clearcoat:.22,clearcoatRoughness:.20,emissive:0x123d36,emissiveIntensity:.11,envMapIntensity:1.04,bumpMap:detailTexture,bumpScale:.010}),
      ...
      red:new THREE.MeshStandardMaterial({color:0x91473b,roughness:.42,metalness:.10,envMapIntensity:.62,bumpMap:detailTexture,bumpScale:.010})
    };
  }
  ...
  function baseScene(scene,renderer){
    ...
    scene.fog=new THREE.FogExp2(0x12100d,.025);
    if(Q) Q.studioEnvironment(scene);
    scene.add(new THREE.HemisphereLight(0xf1dfbd,0x15110d,1.1));
    const key=new THREE.DirectionalLight(0xffedc7,1.38);key.position.set(7,11,8);
    ...
    scene.add(key);
    const rim=new THREE.PointLight(0x67b9ac,.68,28);rim.position.set(-7,4,6);scene.add(rim);
  ...
  window.ByMeliServiceFactory={
    ...
    animate(group,time){
      ...
      if(group?.userData.beams)group.userData.beams.forEach((b,i)=>b.intensity=1.72+Math.sin(time*.90+i*.8)*.10);
      if(group?.userData.led)group.userData.led.material.emissiveIntensity=1.05+Math.sin(time*1.15)*.025;
      if(group?.userData.screen)group.userData.screen.material.emissiveIntensity=1.04+Math.sin(time*1.05)*.020;
  ...
  function initCanvas(initialCanvas){
    ...
    let initialized=false,visible=false,pageVisible=!document.hidden,dragging=false,initAttempts=0,blockedByConnected=false;
    ...
    const viewByType={
      booth:{
        phonePortrait:{yaw:.52,elev:.215,y:.018,cy:.020},phoneLandscape:{yaw:.55,elev:.228,y:.016,cy:.010},
        tablet:{yaw:.58,elev:.238,y:.014,cy:.008},desktop:{yaw:.60,elev:.248,y:.012,cy:.006}
      },
      showroom:{
        phonePortrait:{yaw:.38,elev:.205,y:.024,cy:.016},phoneLandscape:{yaw:.41,elev:.218,y:.022,cy:.009},
        tablet:{yaw:.44,elev:.226,y:.020,cy:.007},desktop:{yaw:.46,elev:.238,y:.018,cy:.005}
      },
      interior:{
        phonePortrait:{yaw:.48,elev:.184,y:.042,cy:.023},phoneLandscape:{yaw:.51,elev:.198,y:.038,cy:.014},
        tablet:{yaw:.54,elev:.208,y:.034,cy:.012},desktop:{yaw:.56,elev:.216,y:.032,cy:.010}
      },
      management:{
        phonePortrait:{yaw:.46,elev:.214,y:.030,cy:.016},phoneLandscape:{yaw:.49,elev:.228,y:.028,cy:.010},
        tablet:{yaw:.52,elev:.238,y:.026,cy:.008},desktop:{yaw:.54,elev:.250,y:.024,cy:.006}
      },
      crowd:{
        phonePortrait:{yaw:.40,elev:.180,y:.050,cy:.027},phoneLandscape:{yaw:.43,elev:.192,y:.046,cy:.016},
        tablet:{yaw:.46,elev:.200,y:.040,cy:.014},desktop:{yaw:.48,elev:.208,y:.038,cy:.012}
      },
      av:{
        phonePortrait:{yaw:.50,elev:.205,y:.040,cy:.018},phoneLandscape:{yaw:.53,elev:.220,y:.036,cy:.012},
        tablet:{yaw:.56,elev:.228,y:.032,cy:.010},desktop:{yaw:.58,elev:.236,y:.030,cy:.008}
      }
    };
    ...
    function qualityRatio(profile){
      ...
      const cap=profile.compact?(profile.high?2.65:(profile.low?1.72:2.28)):(profile.tablet?(profile.high?2.42:2.16):(profile.high?2.28:2.08));
      const budget=profile.compact?(profile.high?4500000:(profile.low?1900000:3200000)):(profile.tablet?(profile.high?5200000:3900000):(profile.high?6400000:4850000));
      return Math.max(1,Math.min(dpr,cap,Math.sqrt(budget/Math.max(1,profile.w*profile.h))));
    }

    function safeFrame(profile){
      if(profile.compact&&profile.portrait)return {left:-.955,right:.955,bottom:-.70,top:.875};
      if(profile.compact)return {left:-.95,right:.95,bottom:-.76,top:.875};
      ...
      if(initialized){
        try{
          scene?.traverse(obj=>{
            ...
            mats.forEach(mat=>{
              if(!mat)return;
              ['map','emissiveMap','bumpMap','normalMap','roughnessMap','metalnessMap','alphaMap','envMap'].forEach(k=>mat[k]?.dispose?.());
              mat.dispose?.();
            });
          });
    ...
    function initialize(){
      if(initialized||!canvas.isConnected||blockedByConnected||window.__BYMELI_CONNECTED_STARTING__)return;
      document.dispatchEvent(new CustomEvent('bymeli:service-scene-claim',{detail:{type}}));
      const profile=computeViewportProfile();
      renderer=createRenderer();
      if(!renderer){
        initAttempts+=1;if(canvas.isConnected)replaceCanvas();
        if(visible&&initAttempts<5&&!blockedByConnected)setTimeout(initialize,Math.min(1000,260*initAttempts));else if(!blockedByConnected)fallback(canvas);
        return;
      }
      ...
      if(Q)Q.configureRenderer(renderer,{exposure:1.20,pixelCap:qualityRatio(profile)});
      else{
      ...
      renderer.setPixelRatio(qualityRatio(profile));renderer.setSize(profile.w,profile.h,false);renderer.setClearColor(0x12100d,1);renderer.physicallyCorrectLights=true;
    ...
    function animateDetails(t){
      ...
      if(root?.userData.beams)root.userData.beams.forEach((b,i)=>b.intensity=1.74+Math.sin(t*.72+i*.8)*.045);
      if(root?.userData.led)root.userData.led.material.emissiveIntensity=1.14+Math.sin(t*.90)*.016;
      if(root?.userData.screen)root.userData.screen.material.emissiveIntensity=1.18+Math.sin(t*.82)*.014;
    ...
    observer=new IntersectionObserver(entries=>{
      ...
      if(visible){
        clearTimeout(disposeTimer);if(!initialized&&!blockedByConnected&&!window.__BYMELI_CONNECTED_STARTING__)initialize();else if(initialized){requestAnimationFrame(()=>resize(false));setTimeout(()=>resize(false),90);startLoop();}
      }else if(initialized){
        stopLoop();disposeTimer=setTimeout(()=>{
          const r=frame.getBoundingClientRect(),vh=window.visualViewport?.height||innerHeight;
          if(r.bottom < -vh*2.4 || r.top > vh*3.2)disposeScene(true);
        },9000);
      }
    ...
    document.addEventListener('bymeli:service-scene-claim',event=>{
      if(event.detail?.type!==type&&initialized){
        // V39 keeps previously initialized service scenes warm instead of
        // destroying their WebGL context, so later sections do not flash
        // LOADING 3D after the connected scene.
        stopLoop();
      }
    });
    document.addEventListener('bymeli:connected-scene-state',event=>{
      blockedByConnected=Boolean(event.detail?.active);
      if(blockedByConnected){
        if(initialized)stopLoop();
      }else if(visible&&!initialized){
        setTimeout(()=>{if(visible&&!blockedByConnected&&!initialized)initialize();},120);
      }else if(visible&&initialized){
        requestAnimationFrame(()=>resize(false));
        startLoop();
      }
    });
    document.addEventListener('bymeli:release-service-scenes',event=>{
      const r=frame.getBoundingClientRect(),vh=window.visualViewport?.height||innerHeight;
      const farOffscreen=r.bottom < -vh*2.2 || r.top > vh*2.8;
      if(event.detail?.offscreenOnly&&farOffscreen&&initialized){
        disposeScene(true);
      }else if(initialized){
        stopLoop();
      }
    ...
    window.ByMeliServiceV27=window.ByMeliServiceV27||{};
    window.ByMeliServiceV26=window.ByMeliServiceV27;
    window.ByMeliServiceV25=window.ByMeliServiceV27;
    window.ByMeliServiceV24=window.ByMeliServiceV27;
    window.ByMeliServiceV20=window.ByMeliServiceV27;
    window.ByMeliServiceV27[type]={
      forceInit(){visible=true;if(!initialized&&!blockedByConnected&&!window.__BYMELI_CONNECTED_STARTING__)initialize();else if(initialized){resize(false);startLoop();}},
      audit(){
