(function(){
  ...
  function initCanvas(initialCanvas){
    ...
    function qualityRatio(profile){
      ...
      const cap=profile.compact?(profile.high?2.50:(profile.low?1.62:2.18)):(profile.tablet?(profile.high?2.34:2.10):(profile.high?2.24:2.04));
      const budget=profile.compact?(profile.high?3900000:(profile.low?1550000:2850000)):(profile.tablet?(profile.high?4850000:3650000):(profile.high?6250000:4700000));
    ...
    document.addEventListener('bymeli:service-scene-claim',event=>{
      if(event.detail?.type!==type&&initialized)disposeScene(true);
    });
    document.addEventListener('bymeli:connected-scene-state',event=>{
      ...
      if(blockedByConnected){
        if(initialized)disposeScene(true);
    ...
    window.ByMeliServiceV26=window.ByMeliServiceV26||{};
    window.ByMeliServiceV25=window.ByMeliServiceV26;
    window.ByMeliServiceV24=window.ByMeliServiceV26;
    window.ByMeliServiceV20=window.ByMeliServiceV26;
    window.ByMeliServiceV26[type]={
