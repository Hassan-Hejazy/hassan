(function(){
  ...
  function initCanvas(initialCanvas){
    ...
    const viewByType={
      booth:{yaw:.54,elev:.215,y:.00},
      showroom:{yaw:-.56,elev:.205,y:.015},
      interior:{yaw:.46,elev:.19,y:.018},
      management:{yaw:-.50,elev:.22,y:.01},
      crowd:{yaw:.42,elev:.185,y:.025},
      av:{yaw:-.47,elev:.215,y:.012}
    };
    const view=viewByType[type]||viewByType.booth;
    ...
    function computeViewportProfile(){
      ...
      let fov=37;
      if(compact)fov=portrait?43.5:(landscape?41.5:42.5);
      else if(tablet)fov=39.5;
      profileCache={w,h,aspect,compact,tablet,portrait,landscape,short,low,fov};
      return profileCache;
    }

    function qualityRatio(profile){
      const dpr=Math.max(1,window.devicePixelRatio||1);
      const cap=profile.compact?(profile.low?1.45:2.25):(profile.tablet?2.15:2.05);
      const budget=profile.compact?(profile.low?1100000:2300000):(profile.tablet?3400000:4700000);
      return Math.max(1,Math.min(dpr,cap,Math.sqrt(budget/Math.max(1,profile.w*profile.h))));
    }

    function safeFrame(profile){
      if(profile.compact)return {left:-.955,right:.955,bottom:-.86,top:.84};
      if(profile.tablet)return {left:-.93,right:.93,bottom:-.86,top:.86};
      return {left:-.91,right:.91,bottom:-.86,top:.87};
    }
    ...
    function calculatePose(yawValue,elevValue,profile){
      ...
      const desiredY=(safe.bottom+safe.top)*.5+(profile.compact?.015:0);
    ...
    function initialize(){
      ...
      renderer.shadowMap.enabled=sceneShadowEnabled;renderer.shadowMap.autoUpdate=sceneShadowEnabled;
      ...
      (factories[type]||factories.booth)(root,materials);root.updateMatrixWorld(true);
      ...
      const yawScale=profile.compact?.94:1;
      yaw=targetYaw=view.yaw*yawScale;elevation=targetElevation=view.elev;
    ...
    function resize(force=false){
      ...
      const baseYaw=view.yaw*(profile.compact?.94:1);
    ...
    function bindInteraction(){
      ...
      canvas.addEventListener('pointermove',e=>{
        ...
        const base=view.yaw*(profileCache?.compact?.94:1);
        targetYaw=clamp(targetYaw+dx*speed,base-1.02,base+1.02);
        targetElevation=clamp(targetElevation+dy*.0018,.10,.36);fitDirty=true;
      },{passive:false});
      const end=e=>{
    ...
    function animateDetails(t){
      ...
      if(root?.userData.beams)root.userData.beams.forEach((b,i)=>b.intensity=1.68+Math.sin(t*1.75+i*.8)*.34);
      if(root?.userData.led)root.userData.led.material.emissiveIntensity=1.05+Math.sin(t*2.6)*.10;
      if(root?.userData.screen)root.userData.screen.material.emissiveIntensity=1.01+Math.sin(t*2.35)*.08;
    ...
    function render(now){
      ...
      const idle=(!dragging&&!reduce)?Math.sin(now*.00018)*.006:0;
      camera.position.copy(cameraPosition(targetPoint,yaw+idle,elevation,distance,tmpPos));camera.up.set(0,1,0);camera.lookAt(targetPoint);
    ...
    window.ByMeliServiceV19=window.ByMeliServiceV19||{};
    window.ByMeliServiceV19[type]={
