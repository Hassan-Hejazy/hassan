(function(){
  ...
  let hub=null,hubCore=null,overviewLinkGroup=null,overviewLinks=[],hubRings=[],hubNodes=[],dataPackets=[],dust=null,finaleBeams=[],finaleSpots=[],finaleGlow=null;
  ...
  function getProfile(){
    ...
    let fov=31.5;
    if(compact)fov=landscape?31.0:(portrait?29.8:30.6);
    else if(tablet)fov=32.2;
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      ...
      motionPixels=p.high?1500000:(p.low?820000:1180000);
      ultraPixels=p.high?7600000:(p.low?2600000:5200000);
      motionCap=p.high?1.78:(p.low?1.36:1.64);
      ultraCap=p.high?3.28:(p.low?2.06:2.96);
      minRatio=p.low?1.04:1.24;
    }else if(p.tablet){
  ...
  const SERVICE_RELATIVE_YAW=[.18,-.12,.12,.14,.08,.16];
  const CAMERA_PRESETS={
    phonePortrait:[
      {yaw:.40,elev:.205,focusY:.014,compY:.018},
      {yaw:.08,elev:.190,focusY:.022,compY:.012},
    ...
    phoneLandscape:[
      ...
      {yaw:.10,elev:.202,focusY:.020,compY:.007},
    ...
    tablet:[
      ...
      {yaw:.12,elev:.208,focusY:.018,compY:.005},
    ...
    desktop:[
      ...
      {yaw:.14,elev:.218,focusY:.016,compY:.003},
  ...
  function configureOverview(){
    if(profile.compact&&profile.portrait){
      overviewPositions=[
        new THREE.Vector3(-4.10,0,6.90),new THREE.Vector3(4.10,0,6.90),
        new THREE.Vector3(-4.55,0,.95),new THREE.Vector3(4.55,0,.95),
        new THREE.Vector3(-4.10,0,-5.00),new THREE.Vector3(4.10,0,-5.00)
      ];
    }else if(profile.tablet||profile.w<1250){
      overviewPositions=[
        new THREE.Vector3(-5.10,0,6.40),new THREE.Vector3(5.10,0,6.40),
        new THREE.Vector3(-5.70,0,.70),new THREE.Vector3(5.70,0,.70),
        new THREE.Vector3(-5.10,0,-5.00),new THREE.Vector3(5.10,0,-5.00)
      ];
    }else{
      overviewPositions=[
        new THREE.Vector3(-7.30,0,6.10),new THREE.Vector3(0,0,6.80),new THREE.Vector3(7.30,0,6.10),
        new THREE.Vector3(-6.20,0,-2.60),new THREE.Vector3(0,0,-3.45),new THREE.Vector3(6.20,0,-2.60)
      ];
    }
    updateOverviewLinks();
  }
  ...
  function buildLuxuryStudio(){
    ...
    const floorSize=profile.compact?82:100;
    const floor=new THREE.Mesh(new THREE.PlaneGeometry(floorSize,floorSize),new THREE.MeshStandardMaterial({color:0x080706,roughness:.82,metalness:.10}));
    floor.rotation.x=-Math.PI/2;floor.position.y=-.02;floor.receiveShadow=Boolean(renderer.shadowMap.enabled);studio.add(floor);
    const inset=new THREE.Mesh(new THREE.PlaneGeometry(profile.compact?46:60,profile.compact?46:60),new THREE.MeshStandardMaterial({color:0x100d0a,roughness:.58,metalness:.16}));
    inset.rotation.x=-Math.PI/2;inset.position.y=-.016;inset.receiveShadow=Boolean(renderer.shadowMap.enabled);studio.add(inset);
    for(const x of (profile.compact?[-10.2,10.2]:[-12.8,12.8])){
      const rail=new THREE.Mesh(new THREE.BoxGeometry(.12,.012,profile.compact?22:28),new THREE.MeshStandardMaterial({color:0xa98949,roughness:.38,metalness:.88}));
      rail.position.set(x,-.010,0);studio.add(rail);
    }
    for(const z of (profile.compact?[-8.4,8.4]:[-10.6,10.6])){
      const line=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?22:28,.012,.12),new THREE.MeshStandardMaterial({color:0xa98949,roughness:.38,metalness:.88}));
      line.position.set(0,-.010,z);studio.add(line);
    }
    const stagePocket=new THREE.Mesh(new THREE.CircleGeometry(profile.compact?4.8:6.1,96),new THREE.MeshStandardMaterial({color:0x15110c,roughness:.46,metalness:.22}));
    stagePocket.rotation.x=-Math.PI/2;stagePocket.position.y=-.014;studio.add(stagePocket);
    ...
    const glow=new THREE.Mesh(new THREE.CircleGeometry(profile.compact?7.2:8.8,profile.low?64:128),new THREE.MeshBasicMaterial({color:0xd6b45d,transparent:true,opacity:.010,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));
    glow.rotation.x=-Math.PI/2;glow.position.y=.012;studio.add(glow);
    if(Q)Q.addContactShadow(studio,renderer,profile.compact?10.8:13.8,profile.compact?.48:.52,.014);
  }

  function buildHub(){
    hub=new THREE.Group();hub.visible=false;root.add(hub);
    hubCore=null;overviewLinkGroup=null;overviewLinks=[];hubRings=[];hubNodes=[];dataPackets=[];dust=null;finaleBeams=[];finaleSpots=[];finaleGlow=null;
    const podium=new THREE.Mesh(new THREE.CylinderGeometry(profile.compact?3.1:4.0,profile.compact?3.4:4.3,.28,48),new THREE.MeshStandardMaterial({color:0x15120e,roughness:.38,metalness:.20}));
    podium.position.y=.10;podium.receiveShadow=Boolean(renderer.shadowMap.enabled);podium.castShadow=true;hub.add(podium);
    const trim=new THREE.Mesh(new THREE.TorusGeometry(profile.compact?3.45:4.35,.045,10,96),new THREE.MeshStandardMaterial({color:0xb7964e,roughness:.34,metalness:.88}));
    trim.rotation.x=Math.PI/2;trim.position.y=.24;hub.add(trim);
    const deck=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?7.8:10.2,.14,profile.compact?3.2:4.0),new THREE.MeshStandardMaterial({color:0xf3efe6,roughness:.56,metalness:.02}));
    deck.position.set(0,.18,-.35);deck.castShadow=true;deck.receiveShadow=true;hub.add(deck);
    const backdrop=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?7.4:9.6,profile.compact?2.6:3.0,.18),new THREE.MeshStandardMaterial({color:0x201a12,roughness:.56,metalness:.10}));
    backdrop.position.set(0,1.60,-1.95);backdrop.castShadow=true;backdrop.receiveShadow=true;hub.add(backdrop);
    const screen=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?4.6:5.9,profile.compact?1.3:1.55,.06),new THREE.MeshStandardMaterial({color:0x0f281d,emissive:0x1a4d37,emissiveIntensity:.75,roughness:.34,metalness:.06}));
    screen.position.set(0,1.66,-1.83);hub.add(screen);
    const brandBar=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?1.2:1.55,.08,.05),new THREE.MeshStandardMaterial({color:0xd2b36a,roughness:.28,metalness:.86}));
    brandBar.position.set(0,2.75,-1.80);hub.add(brandBar);
    const glow=new THREE.Mesh(new THREE.CircleGeometry(profile.compact?4.2:5.4,96),new THREE.MeshBasicMaterial({color:0xd6b45d,transparent:true,opacity:.14,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));
    glow.rotation.x=-Math.PI/2;glow.position.y=.255;hub.add(glow);finaleGlow=glow;
    const beamMat=()=>new THREE.MeshBasicMaterial({color:0xe8d3a2,transparent:true,opacity:.0,depthWrite:false,blending:THREE.AdditiveBlending,side:THREE.DoubleSide,toneMapped:false});
    const lightMat=new THREE.MeshStandardMaterial({color:0xb7964e,roughness:.28,metalness:.88});
    const spots=[[-2.8,3.6,2.8,-.48],[-.9,4.0,3.2,-.18],[.9,4.0,3.2,.18],[2.8,3.6,2.8,.48]];
    spots.forEach(([x,y,z,rz],i)=>{
      const yoke=new THREE.Group();
      const head=new THREE.Mesh(new THREE.CylinderGeometry(.13,.13,.30,20),lightMat);head.rotation.z=Math.PI/2;
      const beam=new THREE.Mesh(new THREE.ConeGeometry(profile.compact?1.2:1.5,profile.compact?4.8:6.0,24,1,true),beamMat());
      beam.position.set(0,-(profile.compact?2.4:3.0),0);beam.rotation.x=Math.PI;beam.userData.base=.16+(i%2)*.04;
      yoke.add(head,beam);yoke.position.set(x,y,z);yoke.rotation.z=rz;yoke.rotation.x=-1.18;hub.add(yoke);finaleSpots.push(yoke);finaleBeams.push(beam);
    });
    for(const x of [-2.8,2.8]){
      const tower=new THREE.Mesh(new THREE.BoxGeometry(.12,2.8,.12),lightMat);tower.position.set(x,1.5,2.6);hub.add(tower);
    }
  }
  ...
  function stageYaw(i){const base=stagePreset(i).yaw;const lead=profile.compact?(profile.portrait?.006:.010):(profile.tablet?.016:.022);return base-lead;}

  function stageElevation(i){ return stagePreset(i).elev-(profile.compact?(profile.portrait?.046:.032):(profile.tablet?.016:.010)); }

  function stageComposition(i){ const p=stagePreset(i);return {x:0,y:p.compY+(profile.compact?(profile.portrait?-.118:-.070):.008)}; }
  ...
  function holdPose(i,u){
    ...
    const orbit=profile.compact?.001:.004;
    ...
    const pose=framedPose(box,yaw,elev,profile.fov,profile.compact?.0004:.0048,stageComposition(i),focus);
    const dolly=1-Math.sin(Math.PI*u)*(profile.compact?.001:.0034);
  ...
  function transitionLayout(i,q){
    ...
    const shift=i===3?(profile.compact?.03:.08):(profile.compact?.06:.12);
    const depth=profile.compact?.016:.04;
    return {
      ...
      aScale:lerp(1,.975,outgoing),bScale:lerp(.975,1,incoming),
  ...
  function bridgePose(i,q){
    ...
    const side=i%2?-1:1;
    pos.y+=Math.sin(Math.PI*t)*(profile.compact?.025:.05);
    pos.x+=Math.sin(Math.PI*t)*side*(profile.compact?.010:.022);
    pos.z+=Math.sin(Math.PI*t)*(profile.compact?.018:.040);
  ...
  function finalOverviewScale(){
    if(profile.compact&&profile.portrait)return .28;
    if(profile.compact)return .31;
    if(profile.tablet||profile.w<1250)return .32;
    return .33;
  }
  ...
  function finalOverviewPose(){
    ...
    const box=new THREE.Box3(new THREE.Vector3(profile.compact?-4.7:-5.8,0,-4.0),new THREE.Vector3(profile.compact?4.7:5.8,4.8,3.8));
    const yaw=portrait?0.0:(twoColumn?.010:.035);
    const elev=portrait?.62:(twoColumn?.56:.50);
    const fov=portrait?(profile.fov-1.2):(twoColumn?(profile.fov-0.8):(profile.fov-0.6));
    return framedPose(box,yaw,elev,fov,portrait?.010:(twoColumn?.012:.012),{x:0,y:portrait?.028:.012},new THREE.Vector3(0,1.35,-.35));
  }
  ...
  function overviewPose(q){
    ...
    const spherical=new THREE.Spherical(
      lerp(sa.radius,sb.radius,t)*(1+Math.sin(Math.PI*t)*(profile.compact?.022:.036)),
      lerp(sa.phi,sb.phi,t)-Math.sin(Math.PI*t)*(profile.compact?.004:.008),
  ...
  function setStationLOD(station,mode){
    ...
    const threshold=profile.compact?1.25:.86;
  ...
  function finalPlacement(i,q){
    ...
    const reveal=smoother(clamp((q-.20-i*.012)/Math.max(.001,.64-(.20+i*.012))));
    const finalPos=overviewCenter.clone().add(overviewPositions[i]);
    const start=overviewCenter.clone().add(overviewPositions[i].clone().multiplyScalar(profile.compact?.42:.46));start.y+=.08;
  ...
  function applyState(state){
    ...
    if(state.mode==='hold'){
      ...
      const finaleReveal=smoother(clamp((state.q-.52)/.38));
      const galleryFade=1-finaleReveal;
      if(state.q<.40){
        ...
        const t=smoother(clamp(state.q/.40));s.wrapper.scale.setScalar(lerp(1,.86,t));s.wrapper.position.y=lerp(0,.04,t);s.accent.intensity=lerp(profile.compact?.42:.55,.10,t);
      }
      if(state.q>.16&&galleryFade>.02){
        stations.forEach((s,i)=>{
          if(i===5&&state.q<.34)return;
          ...
          s.wrapper.position.copy(p.pos);
          s.wrapper.scale.setScalar(p.scale*galleryFade);
          const overviewYaw=SERVICE_RELATIVE_YAW[i]*.42;
          s.wrapper.rotation.y=lerp(stationRotations[i]||0,overviewYaw,p.reveal);
          s.accent.intensity=.02*galleryFade;
          if(finaleReveal>.68)s.wrapper.visible=false;
        });
      }
      if(hub){
        hub.visible=finaleReveal>.02;
        if(hub.visible){
          hub.position.copy(overviewCenter);
          hub.position.y=lerp(.10,0,finaleReveal);
          hub.scale.setScalar(lerp(profile.compact?.82:.88,1,finaleReveal));
        }
      }
    }
  ...
  function updateLighting(stage,state){
    ...
    if(state.mode==='overview')focus.set(0,1.45,-.30);
  ...
  function animateScene(now,state,dt){
    ...
    if(hub&&hub.visible){
      finaleSpots.forEach((spot,i)=>{spot.rotation.z=(i<2?-1:1)*(0.14+Math.sin(t*.52+i)*.03);spot.rotation.x=-1.18+Math.sin(t*.64+i*.7)*.035;});
      finaleBeams.forEach((beam,i)=>{beam.material.opacity=(beam.userData.base||.16)*(.75+.25*Math.sin(t*1.2+i));});
      if(finaleGlow)finaleGlow.material.opacity=.10+.03*Math.sin(t*1.4);
    }
    stations.forEach((s,i)=>{
  ...
  function render(now){
    ...
    // V46 returns to exact scroll mapping so the connected walkthrough has
    // no perceived delay between the user's scroll and the camera response.
  ...
  function init(){
    ...
    const connectedShadows=!profile.low;
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V49 failed:',error);
  ...
  window.ByMeliConnectedV49={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),startupMs:initReadyAt&&initStartedAt?initReadyAt-initStartedAt:null,visible,rafActive:Boolean(rafId),pageVisible,profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV48=window.ByMeliConnectedV49;
  window.ByMeliConnectedV47=window.ByMeliConnectedV49;
  window.ByMeliConnectedV46=window.ByMeliConnectedV49;
  window.ByMeliConnectedV45=window.ByMeliConnectedV49;
  window.ByMeliConnectedV44=window.ByMeliConnectedV49;
  window.ByMeliConnectedV43=window.ByMeliConnectedV49;
  window.ByMeliConnectedV42=window.ByMeliConnectedV49;
  window.ByMeliConnectedV41=window.ByMeliConnectedV49;
  window.ByMeliConnectedV40=window.ByMeliConnectedV49;
  window.ByMeliConnectedV39=window.ByMeliConnectedV49;
  window.ByMeliConnectedV38=window.ByMeliConnectedV49;
  window.ByMeliConnectedV37=window.ByMeliConnectedV49;
  window.ByMeliConnectedV36=window.ByMeliConnectedV49;
  window.ByMeliConnectedV35=window.ByMeliConnectedV49;
  window.ByMeliConnectedV34=window.ByMeliConnectedV49;
  window.ByMeliConnectedV33=window.ByMeliConnectedV49;
  window.ByMeliConnectedV32=window.ByMeliConnectedV49;
  window.ByMeliConnectedV31=window.ByMeliConnectedV49;
  window.ByMeliConnectedV30=window.ByMeliConnectedV49;
  window.ByMeliConnectedV29=window.ByMeliConnectedV49;
  window.ByMeliConnectedV28=window.ByMeliConnectedV49;
  window.ByMeliConnectedV27=window.ByMeliConnectedV49;
  window.ByMeliConnectedV26=window.ByMeliConnectedV49;
  window.ByMeliConnectedV25=window.ByMeliConnectedV49;
  window.ByMeliConnectedV24=window.ByMeliConnectedV49;
  window.ByMeliConnectedV20=window.ByMeliConnectedV49;
