(function(){
  'use strict';
  // V35: cinematic master gallery. Models remain full-size and static while
  // the camera performs a continuous architectural walkthrough.
  ...
  let integratedWorld=null,worldRings=[],worldBeacons=[],worldFrames=[],worldPortals=[],routeRadius=8.6;
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      // V38 prioritizes mobile scroll sync and load speed while still allowing
      // a high-detail settled frame after interaction stops.
      cap=p.high?3.10:(p.low?1.65:2.55);
      targetWidth=p.landscape?3200:1920;
      targetHeight=p.landscape?1920:3200;
      ultraPixels=p.high?6500000:(p.low?2200000:4300000);
      motionPixels=p.high?2200000:(p.low?900000:1500000);
      minRatio=p.low?1.00:1.20;
    }else if(p.tablet){
      cap=p.high?3.10:(p.low?1.85:2.70);
      targetWidth=3200;targetHeight=2000;
      ultraPixels=p.high?7800000:(p.low?3200000:5600000);
      motionPixels=p.high?3600000:(p.low?1700000:2600000);
      minRatio=p.low?1.12:1.34;
    }else{
      cap=p.high?3.20:(p.low?1.90:2.85);
      targetWidth=3840;targetHeight=2160;
      ultraPixels=p.high?9800000:(p.low?4200000:7200000);
      motionPixels=p.high?4200000:(p.low?2200000:3400000);
      minRatio=p.low?1.18:1.45;
    }
  ...
  function useUltraQuality(){
    qualityState='ultra';
    applyRenderRatio(maxRenderRatio,true);
  }
  ...
  function updateAdaptiveQuality(now,frameMs,scrollActive){
    if(scrollActive){
      ...
      if(average>32&&motionRenderRatio>minRenderRatio+.12&&now-lastQualityChange>2400){
    ...
    if(now-lastScrollAt>260&&qualityState!=='ultra')useUltraQuality();
  }
  ...
  function configureRoute(){
    routeRadius=0;
    // V36: widen the architectural gallery so the connected models never
    // overlap in the final overview on mobile or desktop. The previous V35
    // spacing was smaller than the portal plinth diameters.
    const sx=profile.compact?(profile.portrait?8.2:8.8):(profile.tablet?9.4:10.4);
    const sz=profile.compact?(profile.portrait?6.8:6.2):(profile.tablet?7.0:7.8);
    routePositions=[
      new THREE.Vector3(-sx,0, sz),new THREE.Vector3(0,0,sz),new THREE.Vector3(sx,0,sz),
      new THREE.Vector3(sx,0,-sz),new THREE.Vector3(0,0,-sz),new THREE.Vector3(-sx,0,-sz)
    ];
    routeRadius=Math.max(sx,sz);
    overviewCenter.set(0,0,0);
    stationRotations=[0,0,0,Math.PI,Math.PI,Math.PI];
  }


  function configureOverview(){
    overviewPositions=routePositions.map(p=>p.clone());
    updateOverviewLinks();
  }


  function createRenderer(){
    const options={canvas,antialias:(!profile.low&&!profile.compact),alpha:false,premultipliedAlpha:false,powerPreference:'high-performance',precision:profile.low?'mediump':'highp',stencil:false,preserveDrawingBuffer:false,depth:true,logarithmicDepthBuffer:false,desynchronized:false,failIfMajorPerformanceCaveat:false};
  ...
  function makeLiveScreen(i){
    ...
    c.width=profile.compact?(high?960:(profile.low?512:768)):(high?1600:1280);
  ...
  function makeMicroTexture(kind){
    const size=profile.high?(profile.compact?448:896):(profile.low?256:(profile.compact?320:640));
  ...
  function tuneMaterials(materials){
    ...
    if(materials.gold){materials.gold.color.setHex(0xcfa858);materials.gold.roughness=.16;materials.gold.metalness=.90;materials.gold.clearcoat=profile.compact?.62:.74;materials.gold.clearcoatRoughness=.065;materials.gold.envMapIntensity=profile.compact?1.64:1.92;}
  ...
  function buildIntegratedWorld(){
    ...
    integratedWorld=new THREE.Group();integratedWorld.name='cinematic-production-gallery';root.add(integratedWorld);
    worldRings=[];worldBeacons=[];worldFrames=[];worldPortals=[];
    const minX=Math.min(routePositions[0].x,routePositions[5].x)-5.4,maxX=Math.max(routePositions[2].x,routePositions[3].x)+5.4,width=maxX-minX;
    const floorMat=new THREE.MeshPhysicalMaterial({color:0x070807,roughness:.28,metalness:.34,clearcoat:.46,clearcoatRoughness:.16,envMapIntensity:1.15});
    const floorDepth=(Math.max(...routePositions.map(p=>p.z))-Math.min(...routePositions.map(p=>p.z)))+8.8;
    const floor=new THREE.Mesh(new THREE.BoxGeometry(width,.24,floorDepth),floorMat);floor.position.set((minX+maxX)/2,-.18,0);floor.receiveShadow=renderer.shadowMap.enabled;integratedWorld.add(floor);
    const aisleMat=new THREE.MeshPhysicalMaterial({color:0x11120f,roughness:.22,metalness:.52,clearcoat:.38,clearcoatRoughness:.16,envMapIntensity:1.24});
    const aisle=new THREE.Mesh(new THREE.BoxGeometry(width-.7,.035,3.55),aisleMat);aisle.position.set((minX+maxX)/2,-.035,4.15);aisle.receiveShadow=renderer.shadowMap.enabled;integratedWorld.add(aisle);
    const aisleBack=aisle.clone();aisleBack.position.z=-4.15;integratedWorld.add(aisleBack);
    const borderMat=new THREE.MeshBasicMaterial({color:0xdab45f,transparent:true,opacity:.54,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending});
    [-1,1].forEach(sign=>{const line=new THREE.Mesh(new THREE.BoxGeometry(width-.8,.018,.045),borderMat);line.position.set((minX+maxX)/2,.005,4.15+sign*1.74);integratedWorld.add(line);worldRings.push(line);});
    const pathPoints=routePositions.map((p,i)=>new THREE.Vector3(p.x,.08,p.z+(p.z>0?2.95:-2.95)+(i%2?.12:-.12)));
    journeyRoute=new THREE.CatmullRomCurve3(pathPoints,false,'catmullrom',.35);
    const routeGlow=new THREE.Mesh(new THREE.TubeGeometry(journeyRoute,profile.low?80:180,.085,8,false),new THREE.MeshBasicMaterial({color:0x69cdbb,transparent:true,opacity:profile.compact?.025:.05,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));integratedWorld.add(routeGlow);worldRings.push(routeGlow);
    const routeCore=new THREE.Mesh(new THREE.TubeGeometry(journeyRoute,profile.low?80:180,.016,6,false),new THREE.MeshBasicMaterial({color:0xe7c36e,transparent:true,opacity:profile.compact?.20:.38,depthWrite:false,toneMapped:false}));integratedWorld.add(routeCore);worldRings.push(routeCore);
    const bronze=new THREE.MeshPhysicalMaterial({color:0x5b4320,roughness:.24,metalness:.82,clearcoat:.28,clearcoatRoughness:.14,envMapIntensity:1.55});
    const wallMat=new THREE.MeshPhysicalMaterial({color:0x0d100e,roughness:.34,metalness:.26,clearcoat:.12,clearcoatRoughness:.28,envMapIntensity:.92});
    const glowMat=new THREE.MeshBasicMaterial({color:0xe9c66f,transparent:true,opacity:.72,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending});
    routePositions.forEach((p,i)=>{
      const portal=new THREE.Group();portal.position.copy(p);portal.rotation.y=stationRotations[i]||0;integratedWorld.add(portal);
      const architecture=new THREE.Group();architecture.name=`gallery-portal-${i+1}`;portal.add(architecture);worldPortals.push(architecture);
      const back=new THREE.Mesh(new THREE.BoxGeometry(6.55,4.65,.18),wallMat);back.position.set(0,2.30,-3.92);back.receiveShadow=renderer.shadowMap.enabled;architecture.add(back);
      for(let k=-3;k<=3;k++){const slat=new THREE.Mesh(new THREE.BoxGeometry(.035,4.05,.035),bronze);slat.position.set(k*.90,2.26,-3.82);architecture.add(slat);}
      const left=new THREE.Mesh(new THREE.BoxGeometry(.18,5.05,.34),bronze),right=left.clone(),top=new THREE.Mesh(new THREE.BoxGeometry(6.75,.18,.34),bronze);
      left.position.set(-3.28,2.54,-3.72);right.position.set(3.28,2.54,-3.72);top.position.set(0,4.98,-3.72);architecture.add(left,right,top);worldFrames.push(left,right,top);
      const light=new THREE.Mesh(new THREE.BoxGeometry(5.75,.035,.035),glowMat);light.position.set(0,4.85,-3.54);architecture.add(light);
      const plinth=new THREE.Mesh(new THREE.CylinderGeometry(3.45,3.62,.14,profile.low?64:128),new THREE.MeshPhysicalMaterial({color:0x12120f,roughness:.30,metalness:.38,clearcoat:.34,clearcoatRoughness:.18,envMapIntensity:1.22}));plinth.position.y=-.035;plinth.receiveShadow=renderer.shadowMap.enabled;portal.add(plinth);
      const halo=new THREE.Mesh(new THREE.TorusGeometry(3.24,.027,8,profile.low?72:144),new THREE.MeshBasicMaterial({color:i%2?0x69cbbb:0xe7c06a,transparent:true,opacity:.18,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));halo.rotation.x=Math.PI/2;halo.position.y=.055;portal.add(halo);worldRings.push(halo);
      const spotColor=i%2?0x7ddbc9:0xffd98a;
      const spot=new THREE.SpotLight(spotColor,profile.compact?2.9:3.8,18,.46,.58,1.42);spot.position.set(1.45,5.95,1.82);spot.target.position.set(0,1.18,0);spot.castShadow=renderer.shadowMap.enabled&&!profile.compact;portal.add(spot,spot.target);worldBeacons.push(spot);
      const rim=new THREE.PointLight(i%2?0x68cbbb:0xe6b95d,profile.compact?.46:.68,11,2);rim.position.set(-2.25,2.45,-.52);portal.add(rim);worldBeacons.push(rim);
      const numberCanvas=document.createElement('canvas');numberCanvas.width=512;numberCanvas.height=192;const nx=numberCanvas.getContext('2d');nx.clearRect(0,0,512,192);nx.fillStyle='rgba(7,8,7,.92)';nx.fillRect(2,2,508,188);nx.strokeStyle='rgba(224,187,100,.78)';nx.lineWidth=3;nx.strokeRect(7,7,498,178);nx.fillStyle='#f4e7c5';nx.font='700 76px Inter,Arial';nx.textAlign='center';nx.fillText(String(i+1).padStart(2,'0'),256,104);nx.fillStyle='#cfa958';nx.font='500 24px monospace';nx.fillText(serviceCodes[i],256,147);const tex=new THREE.CanvasTexture(numberCanvas);tex.minFilter=THREE.LinearMipmapLinearFilter;tex.magFilter=THREE.LinearFilter;tex.generateMipmaps=true;tex.encoding=THREE.sRGBEncoding;const badge=new THREE.Sprite(new THREE.SpriteMaterial({map:tex,transparent:true,depthWrite:false,toneMapped:false}));badge.position.set(0,4.46,-3.68);badge.scale.set(1.72,.64,1);portal.add(badge);
    });
    // Continuous ceiling architecture and practical lights.
    const ceilingZ=Math.max(...routePositions.map(p=>Math.abs(p.z)))+1.8;
    [-ceilingZ,ceilingZ].forEach(z=>{const beam=new THREE.Mesh(new THREE.BoxGeometry(width,.10,.16),bronze);beam.position.set((minX+maxX)/2,6.18,z);integratedWorld.add(beam);worldFrames.push(beam);});
    for(let x=minX+2;x<maxX-1;x+=3.25){const cross=new THREE.Mesh(new THREE.BoxGeometry(.10,.10,ceilingZ*2.02),bronze);cross.position.set(x,6.18,0);integratedWorld.add(cross);worldFrames.push(cross);if(!profile.compact||Math.round((x-minX)*10)%2===0){const lamp=new THREE.PointLight(0xffe0a8,profile.compact?.10:.26,7,2);lamp.position.set(x,5.9,3.75);integratedWorld.add(lamp);worldBeacons.push(lamp);}}
  }
  ...
  function stageYaw(i){return stationRotations[i]+stagePreset(i).yaw;}

  function stageElevation(i){ return stagePreset(i).elev; }

  function stageComposition(i){if(profile.compact&&profile.portrait)return {x:0,y:-.075};if(profile.compact&&profile.landscape)return {x:0,y:-.018};return {x:0,y:stagePreset(i).compY-.012};}

  function holdPose(i,u){
    const box=stageWorldBox(i);const size=box.getSize(new THREE.Vector3());const preset=stagePreset(i);
    const focus=box.getCenter(new THREE.Vector3());focus.y+=size.y*(preset.focusY+.015);
    const live=smoother(u),direction=i%2?-1:1;
    const yaw=stageYaw(i)+lerp(-.045,.045,live)*direction;
    const elev=Math.max(.16,stageElevation(i)+Math.sin(Math.PI*u)*.018);
    const cinematicFov=profile.compact?(profile.landscape?35.0:34.0):(profile.tablet?31.5:29.5);
    const pose=framedPose(box,yaw,elev,cinematicFov,profile.compact?.018:.020,stageComposition(i),focus);
    const dolly=1-Math.sin(Math.PI*u)*(profile.compact?.025:.038);
  ...
  function transitionLayout(i,q){
    const t=smoother(q);
    return {t,pulse:Math.sin(Math.PI*t),aPos:routePositions[i].clone(),bPos:routePositions[i+1].clone(),aScale:1,bScale:1,showA:q<.49,showB:q>.51};
  }

  function bridgePose(i,q){
    const start=holdCache[i][holdCache[i].length-1],end=holdCache[i+1][0];
    const exitPos=start.pos.clone().sub(start.target).multiplyScalar(1.065).add(start.target);exitPos.y+=profile.compact?.20:.34;
    const entryPos=end.pos.clone().sub(end.target).multiplyScalar(1.065).add(end.target);entryPos.y+=profile.compact?.20:.34;
    if(q<=.49){
      const t=smoother(clamp(q/.49));
      return {pos:start.pos.clone().lerp(exitPos,t),target:start.target.clone(),fov:lerp(start.fov,start.fov+(profile.compact?1.0:1.45),t)};
    }
    if(q>=.51){
      const t=smoother(clamp((q-.51)/.49));
      return {pos:entryPos.clone().lerp(end.pos,t),target:end.target.clone(),fov:lerp(end.fov+(profile.compact?1.0:1.45),end.fov,t)};
    }
    const t=smoother(clamp((q-.49)/.02));
    const pos=exitPos.clone().lerp(entryPos,t);pos.y+=Math.sin(Math.PI*t)*(profile.compact?.55:1.0);
    const target=start.target.clone().lerp(end.target,t);
    return {pos,target,fov:lerp(start.fov,end.fov,t)+(profile.compact?2.0:2.8)};
  }


  function finalOverviewScale(){
    if(profile.compact&&profile.portrait)return .60;
    if(profile.compact)return .64;
    if(profile.tablet)return .70;
    return .76;
  }

  function finalOverviewPose(){
    const scale=finalOverviewScale();
    const union=new THREE.Box3();stations.forEach((_,i)=>union.union(transformedBox(i,routePositions[i],scale,false,stationRotations[i]||0)));
    // V37 overview is cleaner: the six models are slightly reduced so the
    // composition feels curated on mobile and desktop instead of crowded.
    union.expandByPoint(new THREE.Vector3(routePositions[0].x-3.8,-.20,-4.75));
    union.expandByPoint(new THREE.Vector3(routePositions[2].x+3.8,4.25,4.75));
    const focus=union.getCenter(new THREE.Vector3());focus.y=.86;
    const portrait=profile.compact&&profile.portrait;
    const yaw=portrait?.055:(profile.compact?.09:.14);
    const elev=portrait?.61:(profile.compact?.53:(profile.tablet?.50:.47));
    const fov=portrait?33.5:(profile.compact?32.5:(profile.tablet?30.8:28.8));
    return framedPose(union,yaw,elev,fov,profile.compact?.028:.023,{x:0,y:portrait?-.008:-.016},focus);
  }

  function overviewPose(q){
    const start=holdCache[5][holdCache[5].length-1],end=overviewCache.final;const t=smoother(q);
    const p0=start.pos,p3=end.pos;const c1=p0.clone().add(new THREE.Vector3(profile.compact?-1.0:-2.5,profile.compact?2.0:3.2,2.2));const c2=p3.clone().add(new THREE.Vector3(profile.compact?1.0:2.5,-1.0,2.0));
    const pos=cubic(p0,c1,c2,p3,t,new THREE.Vector3());const target=start.target.clone().lerp(end.target,smoother(clamp((t-.08)/.86)));return {pos,target,fov:lerp(start.fov,end.fov,t)};
  }
  ...
  function finalPlacement(i,q){
    const start=.72+i*.018;
    const reveal=smoother(clamp((q-start)/Math.max(.001,.96-start)));
    const scale=lerp(1,finalOverviewScale(),reveal);
    return {pos:routePositions[i].clone(),scale,reveal};
  }


  function transitionVeil(state){
    // V37 intentionally removes the flash/veil effect between models for a
    // cleaner professional walkthrough on mobile and desktop.
    return 0;
  }
  ...
  function applyState(state){
    ...
    if(journeyGroup)journeyGroup.visible=false;
    if(integratedWorld){
      integratedWorld.visible=true;integratedWorld.scale.setScalar(1);
      const overviewMode=state.mode==='overview'&&state.q>.36;
      worldFrames.forEach(f=>f.visible=!overviewMode&&!profile.compact);
      worldPortals.forEach((portal,i)=>{
        if(overviewMode){portal.visible=false;return;}
        if(state.mode==='hold')portal.visible=i===state.stage;
        else if(state.mode==='transition'){portal.visible=(i===state.stage||i===state.next);}
        else portal.visible=i===5;
      });
    }
    if(state.mode==='hold'){
      stations.forEach((s,i)=>{if(i!==state.stage)return;setStationLOD(s,'full');s.wrapper.visible=true;s.detail.visible=true;s.wrapper.position.copy(routePositions[i]);s.wrapper.scale.setScalar(1);s.wrapper.rotation.y=stationRotations[i]||0;s.accent.intensity=profile.compact?1.12:1.38;s.scanner.group.visible=true;});
    }else if(state.mode==='transition'){
      const a=stations[state.stage],b=stations[state.next],layout=transitionLayout(state.stage,state.q);[a,b].forEach((s,k)=>{const show=k?layout.showB:layout.showA;if(!show)return;setStationLOD(s,'full');s.wrapper.visible=true;s.detail.visible=true;s.wrapper.position.copy(routePositions[k?state.next:state.stage]);s.wrapper.scale.setScalar(1);s.wrapper.rotation.y=stationRotations[k?state.next:state.stage]||0;s.accent.intensity=lerp(k?.36:1.18,k?1.18:.36,smoother(state.q));});a.scanner.group.visible=a.wrapper.visible&&state.q<.20;b.scanner.group.visible=b.wrapper.visible&&state.q>.80;
    }else{
      if(state.q<.10){
        const s=stations[5];setStationLOD(s,'full');s.wrapper.visible=true;s.detail.visible=true;s.wrapper.position.copy(routePositions[5]);s.wrapper.scale.setScalar(1);s.wrapper.rotation.y=stationRotations[5]||0;s.accent.intensity=lerp(1.0,.18,smoother(state.q/.10));
      }else if(state.q>.70){
        stations.forEach((s,i)=>{const p=finalPlacement(i,state.q);if(p.reveal<=.004)return;setStationLOD(s,'overview');s.wrapper.visible=true;s.detail.visible=true;s.wrapper.position.copy(p.pos);s.wrapper.scale.setScalar(p.scale);s.wrapper.rotation.y=stationRotations[i]||0;s.accent.intensity=.24*p.reveal;});
      }
    }
    if(hub)hub.visible=false;if(overviewLinkGroup)overviewLinkGroup.visible=false;
  ...
  function updateLighting(stage,state){
    const active=state.mode==='overview'?-1:copyStageFor(state);const focus=active>=0?routePositions[active].clone():overviewCenter.clone();focus.y=1.55;
    if(keyLight){keyLight.position.copy(focus).add(new THREE.Vector3(profile.compact?3.8:5.4,profile.compact?7.0:8.4,profile.compact?4.8:6.4));keyLight.target.position.copy(focus);keyLight.target.updateMatrixWorld();}
    if(rimLight)rimLight.position.copy(focus).add(new THREE.Vector3(-4.8,4.0,-4.0));
    if(fillLight)fillLight.position.copy(focus).add(new THREE.Vector3(3.5,2.8,5.2));
    if(topLight)topLight.position.copy(focus).add(new THREE.Vector3(0,6.4,0));
  }
  ...
  function animateScene(now,state,dt){
    ...
    if(now-screenUpdateAt>(profile.compact?2200:1700)){screenUpdateAt=now;stations.forEach((s,i)=>{if(s.wrapper.visible&&i!==active)drawScreen(s.screen,now,false,overview);});}
  }
  ...
  function resize(){
    ...
    profile=getProfile();configureRoute();configureOverview();buildIntegratedWorld();measureTrack();
  ...
  function render(now){
    ...
    if(renderer.shadowMap.enabled){
      ...
      const shadowInterval=profile.compact?120:(profile.high?76:92);
  ...
  function disposeConnected(replace=true){
    ...
    journeyGroup=journeyRoute=null;journeyPackets=[];integratedWorld=null;worldRings=[];worldBeacons=[];worldFrames=[];worldPortals=[];keyLight=rimLight=fillLight=topLight=null;
  ...
  function init(){
    ...
    const connectedShadows=!profile.compact&&!profile.low;
    ...
    keyLight=new THREE.DirectionalLight(0xffd99a,profile.compact?1.90:2.20);keyLight.position.set(7,10,7);keyLight.castShadow=renderer.shadowMap.enabled;scene.add(keyLight);scene.add(keyLight.target);
    if(keyLight.castShadow){const requested=profile.compact?(profile.high?3072:2048):(profile.high?4096:3072);const size=Math.min(renderer.capabilities.maxTextureSize||4096,requested);keyLight.shadow.mapSize.set(size,size);keyLight.shadow.camera.left=-10;keyLight.shadow.camera.right=10;keyLight.shadow.camera.top=10;keyLight.shadow.camera.bottom=-10;keyLight.shadow.bias=-.00022;keyLight.shadow.normalBias=.022;keyLight.shadow.radius=profile.compact?1.8:2.4;}
    rimLight=new THREE.PointLight(0x72d2bf,profile.compact?1.00:1.34,24,1.9);scene.add(rimLight);
    fillLight=new THREE.PointLight(0xe3b35c,profile.compact?.55:.72,22,2);scene.add(fillLight);
    topLight=new THREE.PointLight(0xffe6bb,profile.compact?.48:.68,18,2);scene.add(topLight);
    updateLighting(0,{mode:'hold',stage:0,next:0,q:0});

    buildIntegratedWorld();buildStations();measureTrack();
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V34 failed:',error);
  ...
  window.ByMeliConnectedV38={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV37=window.ByMeliConnectedV38;
  window.ByMeliConnectedV36=window.ByMeliConnectedV38;
  window.ByMeliConnectedV35=window.ByMeliConnectedV38;
  window.ByMeliConnectedV34=window.ByMeliConnectedV38;
  window.ByMeliConnectedV33=window.ByMeliConnectedV38;
  window.ByMeliConnectedV32=window.ByMeliConnectedV38;
  window.ByMeliConnectedV31=window.ByMeliConnectedV38;
  window.ByMeliConnectedV30=window.ByMeliConnectedV38;
  window.ByMeliConnectedV29=window.ByMeliConnectedV38;
  window.ByMeliConnectedV28=window.ByMeliConnectedV38;
  window.ByMeliConnectedV27=window.ByMeliConnectedV38;
  window.ByMeliConnectedV26=window.ByMeliConnectedV38;
  window.ByMeliConnectedV25=window.ByMeliConnectedV38;
  window.ByMeliConnectedV24=window.ByMeliConnectedV38;
  window.ByMeliConnectedV20=window.ByMeliConnectedV38;
