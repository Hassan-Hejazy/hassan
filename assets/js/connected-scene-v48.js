(function(){
  ...
  function qualityTargets(p){
    ...
    if(p.compact){
      ...
      motionPixels=p.high?1650000:(p.low?850000:1280000);
      ultraPixels=p.high?7200000:(p.low?2520000:5000000);
      motionCap=p.high?1.88:(p.low?1.40:1.72);
      ultraCap=p.high?3.22:(p.low?2.04:2.90);
      minRatio=p.low?1.06:1.28;
    }else if(p.tablet){
  ...
  function configureOverview(){
    if(profile.compact&&profile.portrait){
      overviewPositions=[
        new THREE.Vector3(-4.35,0,7.75),new THREE.Vector3(4.35,0,7.75),
        new THREE.Vector3(-4.85,0,1.10),new THREE.Vector3(4.85,0,1.10),
        new THREE.Vector3(-4.35,0,-5.55),new THREE.Vector3(4.35,0,-5.55)
      ];
    }else if(profile.tablet||profile.w<1250){
      overviewPositions=[
        new THREE.Vector3(-5.45,0,7.10),new THREE.Vector3(5.45,0,7.10),
        new THREE.Vector3(-6.15,0,.80),new THREE.Vector3(6.15,0,.80),
        new THREE.Vector3(-5.45,0,-5.50),new THREE.Vector3(5.45,0,-5.50)
      ];
    }else{
      overviewPositions=[
        new THREE.Vector3(-8.50,0,6.85),new THREE.Vector3(0,0,7.55),new THREE.Vector3(8.50,0,6.85),
        new THREE.Vector3(-7.10,0,-3.10),new THREE.Vector3(0,0,-4.10),new THREE.Vector3(7.10,0,-3.10)
      ];
    }
    updateOverviewLinks();
  }
  ...
  function buildLuxuryStudio(){
    ...
    const floorSize=profile.compact?78:94;
    const floorMat=new THREE.MeshStandardMaterial({color:0x0a0908,roughness:.76,metalness:.14});
    ...
    const inset=new THREE.Mesh(new THREE.PlaneGeometry(profile.compact?42:54,profile.compact?42:54),new THREE.MeshStandardMaterial({color:0x100e0b,roughness:.54,metalness:.18}));
    inset.rotation.x=-Math.PI/2;inset.position.y=-.016;inset.receiveShadow=Boolean(renderer.shadowMap.enabled);studio.add(inset);
    const border=new THREE.Mesh(new THREE.RingGeometry(profile.compact?10.8:13.5,profile.compact?11.0:13.8,96),new THREE.MeshBasicMaterial({color:0xa88a46,transparent:true,opacity:.13,depthWrite:false,toneMapped:false}));
    border.rotation.x=-Math.PI/2;border.position.y=-.012;studio.add(border);
    for(const z of (profile.compact?[-7.6,0,7.6]:[-9.2,0,9.2])){
      const line=new THREE.Mesh(new THREE.BoxGeometry(profile.compact?18.5:24,.012,.12),new THREE.MeshStandardMaterial({color:0xb7964e,roughness:.42,metalness:.86}));
      line.position.set(0,-.010,z);line.castShadow=false;line.receiveShadow=false;studio.add(line);
    }
    ...
    const pool=new THREE.Mesh(new THREE.CircleGeometry(profile.compact?6.2:7.8,profile.low?64:128),new THREE.MeshBasicMaterial({color:0xd6b45d,transparent:true,opacity:.014,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));
    pool.rotation.x=-Math.PI/2;pool.position.y=.012;studio.add(pool);
    if(Q)Q.addContactShadow(studio,renderer,profile.compact?10.4:13.0,profile.compact?.46:.48,.014);
  }
  ...
  function finalOverviewScale(){
    if(profile.compact&&profile.portrait)return .31;
    if(profile.compact)return .34;
    if(profile.tablet||profile.w<1250)return .35;
    return .36;
  }
  ...
  function finalOverviewPose(){
    ...
    stations.forEach((_,i)=>union.union(transformedBox(i,overviewCenter.clone().add(overviewPositions[i]),scale,false,SERVICE_RELATIVE_YAW[i]*.62)));
    const yaw=portrait?0.0:(twoColumn?.016:.075);
    const elev=portrait?.84:(twoColumn?.68:(profile.compact?.57:.50));
    const fov=portrait?(profile.fov+1.5):(twoColumn?(profile.fov+.9):(profile.fov+.2));
    return framedPose(union,yaw,elev,fov,portrait?.012:(twoColumn?.016:.014),{x:0,y:portrait?.002:.000});
  }
  ...
  function finalPlacement(i,q){
    ...
    const reveal=smoother(clamp((q-.26-i*.014)/Math.max(.001,.92-(.26+i*.014))));
    ...
    if(state.mode==='hold'){
      ...
      if(state.q>.20){
        stations.forEach((s,i)=>{
          ...
          const overviewYaw=SERVICE_RELATIVE_YAW[i]*.62;
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V48 failed:',error);
  ...
  window.ByMeliConnectedV48={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),startupMs:initReadyAt&&initStartedAt?initReadyAt-initStartedAt:null,visible,rafActive:Boolean(rafId),pageVisible,profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio,ultraRatio:maxRenderRatio,motionRatio:motionRenderRatio,qualityState}:null})};
  window.ByMeliConnectedV47=window.ByMeliConnectedV48;
  window.ByMeliConnectedV46=window.ByMeliConnectedV48;
  window.ByMeliConnectedV45=window.ByMeliConnectedV48;
  window.ByMeliConnectedV44=window.ByMeliConnectedV48;
  window.ByMeliConnectedV43=window.ByMeliConnectedV48;
  window.ByMeliConnectedV42=window.ByMeliConnectedV48;
  window.ByMeliConnectedV41=window.ByMeliConnectedV48;
  window.ByMeliConnectedV40=window.ByMeliConnectedV48;
  window.ByMeliConnectedV39=window.ByMeliConnectedV48;
  window.ByMeliConnectedV38=window.ByMeliConnectedV48;
  window.ByMeliConnectedV37=window.ByMeliConnectedV48;
  window.ByMeliConnectedV36=window.ByMeliConnectedV48;
  window.ByMeliConnectedV35=window.ByMeliConnectedV48;
  window.ByMeliConnectedV34=window.ByMeliConnectedV48;
  window.ByMeliConnectedV33=window.ByMeliConnectedV48;
  window.ByMeliConnectedV32=window.ByMeliConnectedV48;
  window.ByMeliConnectedV31=window.ByMeliConnectedV48;
  window.ByMeliConnectedV30=window.ByMeliConnectedV48;
  window.ByMeliConnectedV29=window.ByMeliConnectedV48;
  window.ByMeliConnectedV28=window.ByMeliConnectedV48;
  window.ByMeliConnectedV27=window.ByMeliConnectedV48;
  window.ByMeliConnectedV26=window.ByMeliConnectedV48;
  window.ByMeliConnectedV25=window.ByMeliConnectedV48;
  window.ByMeliConnectedV24=window.ByMeliConnectedV48;
  window.ByMeliConnectedV20=window.ByMeliConnectedV48;
