(function(){
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      if(initialized)return;
      try{init();}
      catch(error){console.error('[By Meli] Connected V24 failed:',error);initialized=false;initQueued=false;if(fallback)fallback.style.opacity='.42';if(initAttempts<3)setTimeout(queueInit,700);}
    },420);
  }
  ...
  window.ByMeliConnectedV24={forceInit:queueInit,auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio}:null})};
  window.ByMeliConnectedV20=window.ByMeliConnectedV24;
