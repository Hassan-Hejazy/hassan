(function(){
  ...
  function qualityRatio(p){
    ...
    let cap,budget;
    if(p.compact){
      cap=p.high?2.45:(p.low?1.45:2.05);
      budget=p.high?3000000:(p.low?1200000:2250000);
    }else if(p.tablet){
      cap=p.high?2.35:(p.low?1.50:2.00);
      budget=p.high?4300000:(p.low?1900000:3200000);
    }else{
      cap=p.high?2.20:(p.low?1.50:1.90);
      budget=p.high?5400000:(p.low?2700000:4200000);
    }
    return Math.max(1,Math.min(dpr,cap,Math.sqrt(budget/Math.max(1,p.w*p.h))));
  }

  function configureRoute(){
    const gap=profile.compact?10.4:(profile.tablet?11.2:12.0);
    const bend=profile.compact?1.15:1.55;
    routePositions=Array.from({length:6},(_,i)=>new THREE.Vector3(i*gap,0,(i%2?-.55:.55)*bend));
    overviewCenter.copy(routePositions[5]);
    const cameraYaw=profile.compact?.54:(profile.tablet?.57:.60);
    const desiredViews=profile.compact?[.53,-.50,.51,-.48,.50,-.53]:[.59,-.55,.57,-.53,.55,-.59];
    stationRotations=desiredViews.map(v=>cameraYaw-v);
  }
  ...
  function safeFrame(){
    ...
    if(profile.compact&&profile.portrait)return {left:-.965,right:.965,bottom:-.46,top:.86};
    if(profile.compact&&profile.landscape)return rtl?{left:-.97,right:-.03,bottom:-.78,top:.82}:{left:.03,right:.97,bottom:-.78,top:.82};
    if(profile.compact)return {left:-.96,right:.96,bottom:-.48,top:.84};
    if(profile.tablet)return rtl?{left:-.95,right:.02,bottom:-.82,top:.87}:{left:-.02,right:.95,bottom:-.82,top:.87};
    return rtl?{left:-.955,right:.04,bottom:-.84,top:.88}:{left:-.04,right:.955,bottom:-.84,top:.88};
  }
  ...
  function framedPose(box,yaw,elev,fov,padding=.022){
    ...
    const desiredY=(safe.bottom+safe.top)*.5+(profile.compact?.012:0);
  ...
  function stageYaw(i){
    const base=profile.compact?.54:(profile.tablet?.57:.60);
    const offsets=[-.012,.010,-.008,.012,-.010,.008];
    return base+offsets[i];
  }

  function stageElevation(i){
    const mobile=[.215,.225,.205,.218,.208,.226];
    const tablet=[.235,.245,.225,.238,.228,.246];
    const desktop=[.255,.265,.245,.258,.248,.266];
  ...
  function holdPose(i,u){
    ...
    const yaw=stageYaw(i)+lerp(-.010,.010,live)*direction;
    const elev=stageElevation(i)+Math.sin(Math.PI*u)*.0045;
    const pose=framedPose(box,yaw,elev,profile.fov,profile.compact?.004:.007);
    const dolly=1-Math.sin(Math.PI*u)*.012;
  ...
  function bridgePose(i,q){
    ...
    const lift=profile.compact?.38:(profile.tablet?.54:.70);
    const c1p=start.pos.clone().addScaledVector(delta,.34).addScaledVector(WORLD_UP,lift);
    const c2p=end.pos.clone().addScaledVector(delta,-.34).addScaledVector(WORLD_UP,lift);
    const c1t=start.target.clone().addScaledVector(delta,.32).addScaledVector(WORLD_UP,lift*.15);
    const c2t=end.target.clone().addScaledVector(delta,-.32).addScaledVector(WORLD_UP,lift*.15);
    ...
    return {pos,target,fov:lerp(start.fov,end.fov,t)+Math.sin(Math.PI*t)*(profile.compact?.35:.5)};
  }
  ...
  function transitionVeil(state){
    if(state.mode==='transition')return Math.pow(Math.sin(Math.PI*clamp(state.q)),2.1)*.13;
    if(state.mode==='overview')return Math.pow(Math.sin(Math.PI*clamp(state.q/.58)),1.7)*(state.q<.58?.34:0);
    return 0;
  }
  ...
  function applyState(state){
    ...
    if(state.mode==='hold'){
      ...
      a.wrapper.scale.setScalar(lerp(1,.965,t));b.wrapper.scale.setScalar(lerp(.965,1,t));
      a.accent.intensity=lerp(profile.compact?.84:1.02,.30,t);b.accent.intensity=lerp(.30,profile.compact?.84:1.02,t);
      a.scanner.group.visible=state.q<.64;b.scanner.group.visible=state.q>.36;
    }else{
      if(state.q<.56){
  ...
  function render(now){
    ...
    const lambda=(profile.compact?16.5:14.5)+Math.min(8,gap*24);
  ...
  function init(){
    ...
    initialized=true;initQueued=false;initAttempts+=1;profile=getProfile();configureRoute();configureOverview();
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      if(initialized)return;
      try{init();}
      catch(error){console.error('[By Meli] Connected V19 failed:',error);initialized=false;initQueued=false;if(fallback)fallback.style.opacity='.42';if(initAttempts<3)setTimeout(queueInit,700);}
    },420);
  }
  ...
  window.ByMeliConnectedV19={forceInit:queueInit,auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio}:null})};
