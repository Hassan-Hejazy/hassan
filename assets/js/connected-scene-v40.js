(function(){
  ...
  let screenUpdateAt=0,styleUpdateAt=0,renderRatio=1,maxRenderRatio=1,minRenderRatio=1,lastStateKey='',lastVeil=-1,lastTransit=null;
  ...
  function qualityRatio(p){
    ...
    // Stable supersampling profile. Desktop and tablet target a 4K-class
    // backing buffer while phones retain native-retina density without
    // reallocating the WebGL canvas during the scroll journey.
    let cap,targetWidth,targetHeight,minRatio;
    if(p.compact){
      cap=p.high?3.00:(p.low?1.70:2.50);
      targetWidth=p.landscape?2880:1920;
      targetHeight=p.landscape?1920:2880;
      minRatio=p.low?1.22:1.72;
    }else if(p.tablet){
      cap=p.high?3.08:(p.low?1.80:2.55);
      targetWidth=3440;
      targetHeight=2160;
      minRatio=p.low?1.32:1.78;
    }else{
      cap=p.high?3.25:(p.low?1.90:2.70);
      targetWidth=3840;
      targetHeight=2160;
      minRatio=p.low?1.38:1.90;
    }
    const targetRatio=Math.min(targetWidth/Math.max(1,p.w),targetHeight/Math.max(1,p.h));
    const preferred=Math.max(dpr,Math.min(cap,targetRatio));
    const ratio=Math.max(1,Math.min(cap,targetRatio,preferred));
    return {ratio,minimum:Math.min(ratio,minRatio)};
  }

  function applyRenderRatio(next){
    if(!renderer||!profile)return;
    const rounded=Math.round(clamp(next,minRenderRatio,maxRenderRatio)*20)/20;
    if(Math.abs(rounded-renderRatio)<.045)return;
    renderRatio=rounded;
    renderer.setPixelRatio(renderRatio);
    renderer.setSize(profile.w,profile.h,false);
  }

  function updateAdaptiveQuality(){ /* Stable render density by design. */ }
  ...
  function makeMicroTexture(kind){
    const size=profile.high?(profile.compact?1024:2048):(profile.low?384:(profile.compact?768:1024));
  ...
  function buildStations(){
    ...
    serviceTypes.forEach((type,i)=>{
      ...
      const edgeLimit=profile.low?0:(profile.compact?14:26);let edgeCount=0;
      detail.traverse(o=>{
  ...
  function buildHub(){
    ...
    const coreRingA=new THREE.Mesh(new THREE.TorusGeometry(.58,.022,8,72),new THREE.MeshBasicMaterial({color:0xe8bd65,transparent:true,opacity:.46,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));coreRingA.rotation.x=Math.PI/2;coreRingA.position.y=.29;hubCore.add(coreRingA);
    const coreRingB=new THREE.Mesh(new THREE.TorusGeometry(.88,.010,6,72),new THREE.MeshBasicMaterial({color:0x6fd0bd,transparent:true,opacity:.18,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));coreRingB.rotation.x=Math.PI/2;coreRingB.position.y=.32;hubCore.add(coreRingB);
    ...
    const coreBeam=new THREE.Mesh(new THREE.CylinderGeometry(.020,.05,2.15,16,1,true),new THREE.MeshBasicMaterial({color:0xe9c46d,transparent:true,opacity:.10,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending,side:THREE.DoubleSide}));coreBeam.position.y=1.38;hubCore.add(coreBeam);
    ...
    ringSpecs.forEach((spec,i)=>{
      ...
      ring.rotation.x=Math.PI/2;ring.position.y=.30+i*.012;ring.visible=false;hub.add(ring);hubRings.push(ring);
    });
    for(let i=0;i<6;i++){
      ...
      node.add(core,orbit);node.position.set(Math.cos(a)*6.48,.33,Math.sin(a)*6.48);node.visible=false;hub.add(node);hubNodes.push(node);
    }
    ...
    for(let i=0;i<packetCount;i++){
      ...
      packet.userData={offset:i/packetCount,speed:.045+(i%3)*.004,curve:route};packet.visible=false;hub.add(packet);dataPackets.push(packet);
    }
    ...
    dust=new THREE.Points(geo,new THREE.PointsMaterial({map:dotTex,size:profile.compact?.050:.065,transparent:true,opacity:.05,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));dust.visible=false;dust.frustumCulled=false;root.add(dust);
  }
  ...
  function buildJourney(){
    ...
    const routeMat=new THREE.MeshBasicMaterial({color:0xd9ae59,transparent:true,opacity:.07,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending});
    const routeGlowMat=new THREE.MeshBasicMaterial({color:0x67c8b7,transparent:true,opacity:.018,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending});
    ...
    for(let i=0;i<count;i++){
      const packet=new THREE.Mesh(new THREE.SphereGeometry(profile.compact?.038:.048,10,8),new THREE.MeshBasicMaterial({color:i%2?0x6fd0bd:0xf0c66e,transparent:true,opacity:.34,depthWrite:false,toneMapped:false,blending:THREE.AdditiveBlending}));
  ...
  function transitionLayout(i,q){
    ...
    const side=i%2?-1:1;
    const shift=profile.compact?1.86:(profile.tablet?2.12:2.38);
    const depth=profile.compact?.52:(profile.tablet?.64:.76);
    const aPos=new THREE.Vector3(-side*shift*t,0,-depth*t);
    const bPos=new THREE.Vector3(side*shift*(1-t),0,depth*(1-t));
    const minimum=profile.compact?.79:.82;
    const scale=lerp(1,minimum,pulse);
  ...
  function bridgePose(i,q){
    ...
    const t=layout.t;
    const union=transformedBox(i,layout.aPos,layout.aScale,false,stationRotations[i]||0);
    union.union(transformedBox(i+1,layout.bPos,layout.bScale,false,stationRotations[i+1]||0));
    const startPreset=stagePreset(i),endPreset=stagePreset(i+1);
    let yawDelta=endPreset.yaw-startPreset.yaw;
    while(yawDelta>Math.PI)yawDelta-=Math.PI*2;
    while(yawDelta<-Math.PI)yawDelta+=Math.PI*2;
    const yaw=startPreset.yaw+yawDelta*t;
    const elev=lerp(startPreset.elev,endPreset.elev,t)+Math.sin(Math.PI*t)*(profile.compact?.018:.024);
    const focus=union.getCenter(new THREE.Vector3());
    const size=union.getSize(new THREE.Vector3());
    focus.y+=size.y*lerp(startPreset.focusY,endPreset.focusY,t);
    const composition={x:0,y:lerp(startPreset.compY,endPreset.compY,t)};
    const pose=framedPose(union,yaw,elev,profile.fov+(profile.compact?.18:.10),profile.compact?.012:.015,composition,focus);
    const breathing=1+Math.sin(Math.PI*t)*(profile.compact?.018:.025);
    pose.pos.sub(pose.target).multiplyScalar(breathing).add(pose.target);
    return pose;
  }
  ...
  function applyState(state){
    ...
    if(state.mode==='hold'){
      ...
      a.wrapper.visible=state.q<.94;b.wrapper.visible=state.q>.06;
      ...
      a.scanner.group.visible=state.q<.34;b.scanner.group.visible=state.q>.66;
    }else{
      if(state.q<.56){
    ...
    if(hub){
      const inOverview=false;
      const hs=1;
      hub.position.copy(overviewCenter);hub.scale.setScalar(hs);
      if(hubCore)hubCore.visible=false;
      if(overviewLinkGroup){overviewLinkGroup.visible=false;}
    }
  ...
  function resize(){
    ...
    {const q=qualityRatio(profile);maxRenderRatio=q.ratio;minRenderRatio=q.minimum;renderRatio=maxRenderRatio;}renderer.setPixelRatio(renderRatio);renderer.setSize(profile.w,profile.h,false);
  ...
  function render(now){
    ...
    // Native scrolling is already interpolated by the browser. Use the exact
    // normalized scroll value so every pixel maps to one deterministic 3D frame
    // on phones and desktops, with no camera lag or delayed catch-up.
  ...
  function init(){
    ...
    {const q=qualityRatio(profile);maxRenderRatio=q.ratio;minRenderRatio=q.minimum;renderRatio=maxRenderRatio;}
    ...
    edgeMaterial=new THREE.LineBasicMaterial({color:0xf5e8c9,transparent:true,opacity:profile.compact?.045:.060,depthWrite:false,toneMapped:false});
  ...
  function queueInit(){
    ...
    setTimeout(()=>{
      ...
      try{init();}
        ...
        console.error('[By Meli] Connected V30 failed:',error);
  ...
  window.ByMeliConnectedV40={forceInit:queueInit,dispose:()=>disposeConnected(true),auditAt,renderAt,getState:()=>({initialized,progress,target:targetProgress,state:timeline(progress),profile:profile?{w:profile.w,h:profile.h,tier:profile.tier,pixelRatio:renderRatio}:null})};
  window.ByMeliConnectedV39=window.ByMeliConnectedV40;
  window.ByMeliConnectedV38=window.ByMeliConnectedV40;
  window.ByMeliConnectedV37=window.ByMeliConnectedV40;
  window.ByMeliConnectedV36=window.ByMeliConnectedV40;
  window.ByMeliConnectedV35=window.ByMeliConnectedV40;
  window.ByMeliConnectedV34=window.ByMeliConnectedV40;
  window.ByMeliConnectedV33=window.ByMeliConnectedV40;
  window.ByMeliConnectedV32=window.ByMeliConnectedV40;
  window.ByMeliConnectedV31=window.ByMeliConnectedV40;
  window.ByMeliConnectedV30=window.ByMeliConnectedV40;
  window.ByMeliConnectedV29=window.ByMeliConnectedV40;
  window.ByMeliConnectedV28=window.ByMeliConnectedV40;
  window.ByMeliConnectedV27=window.ByMeliConnectedV40;
  window.ByMeliConnectedV26=window.ByMeliConnectedV40;
  window.ByMeliConnectedV25=window.ByMeliConnectedV40;
  window.ByMeliConnectedV24=window.ByMeliConnectedV40;
  window.ByMeliConnectedV20=window.ByMeliConnectedV40;
