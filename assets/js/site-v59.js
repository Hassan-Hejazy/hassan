(function(){
  ...
  const ariaTranslatable=Array.from(document.querySelectorAll('[data-aria-en][data-aria-ar]'));
  const altTranslatable=Array.from(document.querySelectorAll('[data-alt-en][data-alt-ar]'));
  const metaDescription=document.querySelector('meta[name="description"]');
  const pageMeta={
    en:{title:'By Meli Production — Exhibition Booths, Events & 3D Production',description:'By Meli designs, manufactures and delivers exhibition booths, branded spaces, event interiors, live production, project management, crowd flow and audio visual systems across Saudi Arabia.'},
    ar:{title:'باي ملي للإنتاج — أجنحة المعارض والفعاليات والإنتاج ثلاثي الأبعاد',description:'تصمم باي ملي وتنفذ أجنحة المعارض ومساحات العلامات التجارية وديكورات الفعاليات والإنتاج الحي وإدارة المشاريع والحشود والأنظمة السمعية والبصرية في جميع مناطق المملكة.'}
  };
  const wecanSvg=document.querySelector('.wecan-mask-svg');
  const wecanMask=document.getElementById('wecanCutoutMask');
  const wecanCover=document.querySelector('.wecan-mask-cover');
  ...
  function syncWecanSvgLayout(){
    ...
    if(portrait){
      ...
      wecanMask?.setAttribute('width','900');wecanMask?.setAttribute('height','1600');
      wecanCover?.setAttribute('width','900');wecanCover?.setAttribute('height','1600');
      wecanTexts.forEach(el=>{el.setAttribute('x','450');el.setAttribute('y','860');if(ar){el.removeAttribute('textLength');el.removeAttribute('lengthAdjust');}else{el.setAttribute('textLength','790');el.setAttribute('lengthAdjust','spacingAndGlyphs');}});
    }else{
      wecanSvg.setAttribute('viewBox','0 0 1600 900');
      wecanSvg.setAttribute('preserveAspectRatio','xMidYMid slice');
      wecanMask?.setAttribute('width','1600');wecanMask?.setAttribute('height','900');
      wecanCover?.setAttribute('width','1600');wecanCover?.setAttribute('height','900');
      wecanTexts.forEach(el=>{el.setAttribute('x','800');el.setAttribute('y','535');if(ar){el.removeAttribute('textLength');el.removeAttribute('lengthAdjust');}else{el.setAttribute('textLength','1230');el.setAttribute('lengthAdjust','spacingAndGlyphs');}});
    }
  }
  ...
  function setLanguage(lang){
    ...
    root.lang=lang;root.dir=lang==='ar'?'rtl':'ltr';root.classList.toggle('lang-ar',lang==='ar');
    translatable.forEach(el=>{const value=el.getAttribute(`data-${lang}`);if(value!==null)el.textContent=value});
    ariaTranslatable.forEach(el=>{const value=el.getAttribute(`data-aria-${lang}`);if(value!==null)el.setAttribute('aria-label',value)});
    altTranslatable.forEach(el=>{const value=el.getAttribute(`data-alt-${lang}`);if(value!==null)el.setAttribute('alt',value)});
    document.title=pageMeta[lang].title;if(metaDescription)metaDescription.setAttribute('content',pageMeta[lang].description);
    syncWecanSvgLayout();
