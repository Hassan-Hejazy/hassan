(function(){
  ...
  let cameraSamples=[],overviewStart=.82,overviewPositions=[],overviewScale=.56;
  let profile=null;
  ...
  function getProfile(){
    ...
    const w=Math.max(1,Math.round(rect.width||sticky.clientWidth||innerWidth));
    const h=Math.max(1,Math.round(rect.height||sticky.clientHeight||innerHeight));
    const aspect=w/h;
    ...
    const fov=compact?(portrait?43.5:41.5):(tablet?(portrait?42:40):38.5);
  ...
  function choosePixelRatio(p){
    ...
    let cap,budget;
    if(p.compact){
      cap=p.tier==='high'?2.75:(p.tier==='balanced'?2.3:1.75);
      budget=p.tier==='high'?5200000:(p.tier==='balanced'?3300000:1850000);
    }else if(p.tablet){
      cap=p.tier==='high'?2.55:(p.tier==='balanced'?2.2:1.7);
      budget=p.tier==='high'?7000000:(p.tier==='balanced'?4700000:2600000);
    }else{
      cap=p.tier==='high'?2.5:(p.tier==='balanced'?2.15:1.65);
      budget=p.tier==='high'?8300000:(p.tier==='balanced'?5900000:3200000);
    }
    return Math.max(1,Math.min(dpr,cap,Math.sqrt(budget/Math.max(1,p.w*p.h))));
  }

  function minimumPixelRatio(p){
    if(p.compact)return p.low?1.35:1.6;
    return p.low?1.25:1.5;
  }
  ...
  function monitorQuality(dt,now){
    ...
    frameMs=lerp(frameMs,dt*1000,.045);
    if(now-startedAt<2400||now-qualityCheckedAt<2200)return;
    qualityCheckedAt=now;
    const minScale=Math.min(1,minimumPixelRatio(profile)/Math.max(1,basePixelRatio));
    let next=qualityScale;
    if(frameMs>25.5)next=Math.max(minScale,qualityScale-.12);
    else if(frameMs<16.4&&qualityScale<1)next=Math.min(1,qualityScale+.075);
    if(Math.abs(next-qualityScale)>.02){qualityScale=next;applyPixelRatio();}
  }
  ...
  function buildStations(){
    ...
    types.forEach((type,i)=>{
      ...
      const localBox=detailBox.clone().expandByScalar(.22);
      const cameraRadius=profile.compact?3.55:4.05;
      localBox.union(new THREE.Box3(new THREE.Vector3(-cameraRadius,0,-cameraRadius),new THREE.Vector3(cameraRadius,Math.max(4.75,detailBox.max.y+.22),cameraRadius)));
  ...
  function safeFrame(){
    ...
    if(profile.compact&&profile.portrait)return {left:-.90,right:.90,bottom:-.38,top:.80};
    if(profile.compact&&profile.landscape)return rtl?{left:-.92,right:.05,bottom:-.80,top:.80}:{left:-.05,right:.92,bottom:-.80,top:.80};
    if(profile.tablet)return rtl?{left:-.93,right:.18,bottom:-.76,top:.82}:{left:-.18,right:.93,bottom:-.76,top:.82};
    return rtl?{left:-.95,right:.17,bottom:-.78,top:.84}:{left:-.17,right:.95,bottom:-.78,top:.84};
  }
  ...
  function framedPose(box,yaw,elev,fov,padding){
    ...
    for(let pass=0;pass<3;pass++){
      ...
      target.addScaledVector(right,-(desiredX-currentX)*halfH*.92);
      target.addScaledVector(camUp,-(desiredY-currentY)*halfV*.92);
    }
    distance=fitDistance(box,target,yaw,elev,fov,padding);
  ...
  function stationPose(i,variant=0){
    ...
    const mobileYaw=[.47,-.43,.49,-.40,.44,-.48];
    const tabletYaw=[.56,-.51,.57,-.47,.51,-.54];
    const desktopYaw=[.62,-.57,.63,-.52,.56,-.59];
    const bases=profile.compact?mobileYaw:(profile.tablet?tabletYaw:desktopYaw);
    const elev=profile.compact?(profile.portrait?.205:.18):(profile.tablet?.225:.245);
    const orbit=(variant-1)*(profile.compact?.038:.052)*(i%2?-.92:1);
    const baseYaw=bases[i]+orbit;const fov=profile.fov+(variant===1?-.45:(variant===2?.18:0));
    const offsets=profile.compact?[-.14,-.07,0,.07,.14]:[-.12,-.06,0,.06,.12];
    let best=null;
    offsets.forEach(offset=>{
      const yaw=baseYaw+offset;const framed=framedPose(box,yaw,elev,fov,profile.compact?.026:.024);
      const score=framed.distance*(1+Math.abs(offset)*.055);
      if(!best||score<best.score)best={...framed,yaw,score};
    });
    return {pos:best.pos,target:best.target,fov,stage:i,overview:false,yaw:best.yaw};
  }

  function stationBaseYaw(i){
    const mobile=[.45,-.42,.47,-.39,.43,-.46];
    const tablet=[.53,-.49,.54,-.46,.49,-.51];
    const desktop=[.58,-.54,.59,-.49,.52,-.55];
    return (profile.compact?mobile:(profile.tablet?tablet:desktop))[i];
  }

  function stationPoseAt(i,local=0){
    const box=boxAt(i,centers[i],1);
    const phase=clamp(local/.62);
    const direction=i%2?-1:1;
    const orbit=Math.sin(phase*Math.PI)*direction*(profile.compact?.042:.055);
    const yaw=stationBaseYaw(i)+orbit;
    const elev=profile.compact?(profile.portrait?.205:.18):(profile.tablet?.22:.24);
    const fov=profile.fov- Math.sin(phase*Math.PI)*.35;
    const framed=framedPose(box,yaw,elev,fov,profile.compact?.045:.04);
  ...
  function transitionPlacement(i,q){
    ...
    const e=smoother(clamp(q));
    const arc=Math.sin(Math.PI*e)*(profile.compact?.58:.78);
    // Both environments travel along the same production route and exchange
    // visual weight. They begin and end at the same anchors, eliminating the
    // camera snaps caused by a disappearing model remaining far behind.
    const activePos=pa.clone().lerp(pb,e).addScaledVector(side,-arc*.48);
    const nextPos=pa.clone().lerp(pb,e).addScaledVector(side,arc*.48);
    return {
      activePos,
      nextPos,
      activeScale:lerp(1,.075,e),
      nextScale:lerp(.075,1,e),
      ease:e
    };
  }

  function transitionPose(i,q=.5){
    const placement=transitionPlacement(i,q);
    const all=new THREE.Box3();
    all.union(boxAt(i,placement.activePos,placement.activeScale));
    all.union(boxAt(i+1,placement.nextPos,placement.nextScale));
    const startYaw=stationBaseYaw(i),endYaw=stationBaseYaw(i+1);
    const angleMix=(a,b,t)=>a+Math.atan2(Math.sin(b-a),Math.cos(b-a))*t;
    const midYaw=angleMix(startYaw,endYaw,.5)+(i%2?-.10:.10);
    const qn=clamp(q);const first=smoother(clamp(qn*2));const second=smoother(clamp((qn-.5)*2));
    const yaw=qn<.5?angleMix(startYaw,midYaw,first):angleMix(midYaw,endYaw,second);
    const stationElev=profile.compact?(profile.portrait?.205:.18):(profile.tablet?.22:.24);
    const travelElev=profile.compact?(profile.portrait?.225:.195):.255;
    const elev=qn<.5?lerp(stationElev,travelElev,first):lerp(travelElev,stationElev,second);
    const travelFov=profile.fov+(profile.compact?.7:.85);
    const fov=qn<.5?lerp(profile.fov,travelFov,first):lerp(travelFov,profile.fov,second);
    const framed=framedPose(all,yaw,elev,fov,profile.compact?.055:.05);
    return {pos:framed.pos,target:framed.target,fov,stage:i,overview:false,bridge:true};
  }
  ...
  function overviewPlacement(i,q){
    ...
    const side=(i%2?1:-1)*(profile.compact?.18:.25)*Math.sin(Math.PI*e);
    const position=origin.clone().lerp(overviewPositions[i],e);position.x+=side;
    const startScale=i===5?1:.008;
    return {position,scale:lerp(startScale,overviewScale,e)};
  }
  ...
  function overviewPose(amount=1){
    ...
    const qn=clamp(amount),e=smoother(qn);
    const angleMix=(a,b,t)=>a+Math.atan2(Math.sin(b-a),Math.cos(b-a))*t;
    const endYaw=profile.compact?(profile.portrait?.075:.25):.39;
    const yaw=angleMix(stationBaseYaw(5),endYaw,e);
    const startElev=profile.compact?(profile.portrait?.205:.18):(profile.tablet?.22:.24);
    const endElev=profile.compact?(profile.portrait?.39:.33):.325;
    const elev=lerp(startElev,endElev,e);
    const endFov=profile.fov+(profile.compact?.75:.5);
    const fov=lerp(profile.fov,endFov,e);
    const framed=framedPose(all,yaw,elev,fov,profile.compact?.06:.055);
  ...
  function timelineInfo(p){
    ...
    const span=overviewStart/6;const raw=p/span;const stage=clamp(Math.floor(raw),0,5);const local=raw-stage;
    const transition=stage<5&&local>.62;
    return {stage,local,transition,transitionQ:transition?clamp((local-.62)/.38):0,next:Math.min(5,stage+1)};
  }
  ...
  function buildCameraPath(){
    cameraSamples=[];
    const count=profile.low?181:(profile.compact?241:281);
    for(let i=0;i<count;i++){
  ...
  function updateVisibility(state){
    ...
    const transitionEase=state.transitionQ||0;const placement=state.transition?transitionPlacement(active,transitionEase):null;
    stations.forEach((s,i)=>{
      if(overview){
        const reveal=smoother(clamp((state.overviewQ-i*.035)/.58));
        const p=overviewPlacement(i,state.overviewQ);
        s.wrapper.visible=i===5||reveal>.01;s.wrapper.position.copy(p.position);s.wrapper.scale.setScalar(p.scale);
        const fullDetail=!profile.low||i===5;
        s.detail.visible=s.wrapper.visible&&fullDetail;s.proxy.visible=s.wrapper.visible&&!fullDetail;
        s.light.intensity=.14+.34*reveal;
      }else{
        const activeVisible=i===active&&(!state.transition||placement.activeScale>.14);
        const incoming=i===next&&state.transition&&placement.nextScale>.14;
        const show=activeVisible||incoming;
        s.wrapper.visible=show;
        if(activeVisible&&placement){s.wrapper.position.copy(placement.activePos);s.wrapper.scale.setScalar(placement.activeScale);}
        else if(incoming&&placement){s.wrapper.position.copy(placement.nextPos);s.wrapper.scale.setScalar(placement.nextScale);}
        else{s.wrapper.position.copy(centers[i]);s.wrapper.scale.setScalar(1);}
        let detailAllowed;
        if(profile.compact&&state.transition){
          detailAllowed=(activeVisible&&transitionEase<.64)||(incoming&&transitionEase>.48);
        }else{
          detailAllowed=activeVisible||(!profile.low&&incoming)||(incoming&&transitionEase>.62);
        }
        s.detail.visible=show&&detailAllowed;s.proxy.visible=show&&!s.detail.visible;
        s.light.intensity=i===active?lerp(1.55,.48,transitionEase):(incoming?lerp(.36,1.42,transitionEase):0);
      }
      const ring=stationRings[i];if(ring)ring.material.opacity=overview?.36:(i===active?lerp(.84,.38,transitionEase):(i===next?lerp(.22,.78,transitionEase):.1));
      const label=stationLabels[i];if(label)label.material.opacity=overview?.62:(i===active?lerp(.98,.38,transitionEase):(i===next?lerp(.16,.92,transitionEase):.08));
      const halo=stationHalos[i];if(halo)halo.material.opacity=overview?.3:(i===active?lerp(.62,.24,transitionEase):(i===next?lerp(.12,.56,transitionEase):.06));
    });
    dataOrbits.forEach(o=>{const i=o.userData.station;o.material.opacity=overview?.18:(i===active?lerp(.34,.12,transitionEase):(i===next?lerp(.04,.3,transitionEase):0));});
  }
  ...
  function resize(){
    ...
    if(scene?.fog)scene.fog.density=profile.compact?.0095:.0076;
    buildCameraPath();readProgress();
  ...
  function animateScene(now,state){
    ...
    dataOrbits.forEach((orbit,i)=>{orbit.rotation.y+=orbit.userData.speed*.008;orbit.rotation.z+=Math.sin(t*.38+i)*.00035;});
    ...
    if(cinemaKey){const desired=tmp0.copy(focus).add(new THREE.Vector3(profile.compact?4.5:6.8,profile.compact?7:9.2,profile.compact?4.2:6.4));cinemaKey.position.lerp(desired,.08);cinemaKey.intensity=state.overview?.8:1.25;}
    if(cinemaRim){const desired=tmp1.copy(focus).add(new THREE.Vector3(profile.compact?-4.2:-6.4,profile.compact?4.4:5.8,-4.6));cinemaRim.position.lerp(desired,.075);cinemaRim.intensity=state.overview?.55:.82;}
  }
  ...
  function render(now){
    ...
    const rate=reduced?100:(profile.compact?17:15.5);
    const requested=(targetProgress-progress)*(1-Math.exp(-rate*dt));
    const frameScale=clamp(dt*60,.45,1.55);
    const maxStep=(profile.compact?.006:.0072)*frameScale;
    progress+=reduced?requested:clamp(requested,-maxStep,maxStep);
    if(Math.abs(targetProgress-progress)<.000025)progress=targetProgress;
    ...
    updateVisibility(state);copyBox?.classList.toggle('in-transit',Boolean(state.transition));updateCopy(state);animateScene(now,state);
    sticky.style.setProperty('--connected-progress',progress.toFixed(4));sticky.style.setProperty('--connected-progress-pct',(progress*100).toFixed(2)+'%');if(indicator)indicator.style.opacity=progress>.93?'0':'1';
  ...
  function queueInit(){
    ...
    setTimeout(()=>{if(!initialized){try{init();}catch(error){console.error('[By Meli] Connected V13 failed:',error);initialized=false;initQueued=false;if(fallback)fallback.style.opacity='.46';if(initAttempts<3)setTimeout(queueInit,720);}}},260);
  }
