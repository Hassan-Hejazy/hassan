(function(){
  ...
  function render(now){
    ...
    if(renderDirty||performance.now()-lastScrollAt<72)raf=requestAnimationFrame(render);
  }
  ...
  function init(){
    ...
    try{
      ...
      console.error('Connected V56 init failed:',error);
      initialized=false;
  ...
  window.ByMeliConnectedV56={forceInit,renderAt,auditAt,getState:()=>({initialized,progress,target:targetProgress,profile:profile?{w:profile.w,h:profile.h,pixelRatio:stablePixelRatio(profile),tier:profile.tier}:null})};
  for(const v of [55,54,53,52,51,50,49,48,47,46,45,44,43,42,41,40,39,38,37,36,35,34,33,32,31,30,29,28,27,26,25,24,20])window['ByMeliConnectedV'+v]=window.ByMeliConnectedV56;
